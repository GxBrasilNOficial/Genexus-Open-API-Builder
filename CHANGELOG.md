...
- `B096` (Sprint 9 / Fase 2): plano de SDTs hierárquicos por contrato quando `ApiPlan.Levels` tem filhos; naming `sdt<Tx>_API_<Papel>_<caminho>` com desambiguação por `LevelOrder` e encurtamento a 128 via hash; marcador `<Subnível>Replace` no Update; pós-ordem em `OwnSdts`; `ListResponse.Items` permanece coleção de `Response`; ouro em `Tests/SdtHierarchicalPlan/Baselines/` e gate `tests.sdtHierarchicalPlan`. A trava `Test-OpenApiClientContractValidity.ps1` passa a ler também `ApiPlanSdtHierarchicalNaming.cs` para `_API_CreateRequest_`, `_API_UpdateRequest_` e `_API_Response_` (sem `_API_ListResponse_Item`). Wizard flat e `ListResponse_Item` fora deste recorte. Evidência: `Docs/Implementation/2026-08-26-B096-SDTS-HIERARQUICOS.md`.
...
- Checker e `AGENTS.md`: `currentFront`/`manualRequired` documentados como exclusivos de spike `B000`–`B006`; lista vazia com próxima ação `B007+` (ex. `B097`) é o contrato, não falso verde, e não substitui a revisão semântica.
- Documentação B096: `10` declara o plano de SDT consumindo `Levels` quando há filhos; `ListResponse_Item` e contadores permanecem B098+; o `11` registra o limite 128 e o encurtamento da folha (>32 → hash de 8 hex); o `08` e o `13` deixam de afirmar que a geração não consome a árvore; o `15`, o `26` e o `28` separam o contrato-alvo (B098 / Fase 7) do plano já emitido; a especificação de subníveis alinha a ordem de execução, o critério 2 (`Tests/SdtHierarchicalPlan/`) e o critério 9 (trava OpenAPI também lê `ApiPlanSdtHierarchicalNaming.cs`; `_API_ListResponse_Item` ainda não).
