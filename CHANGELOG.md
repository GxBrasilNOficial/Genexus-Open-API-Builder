...
- Documentos de fundação reconciliados com a semântica de obrigatoriedade efetivamente entregue: `B076` deixa de prometer distinção entre membro JSON ausente e vazio, `false` ou `0`, comprovadamente inviável sem comando `csharp`, e passa a descrever a recusa com 400 de campo obrigatório não preenchido. O documento 06 recebeu nota de revisão preservando o enunciado original, os quatro caminhos descartados e a limitação assumida; o documento 24 e o gate transversal 6 dos documentos 09, 15 e 24 foram alinhados à mesma redação. A restrição de não expor campos públicos `Specified` no contrato permanece inalterada.
