...
- `S-B111 F3` — quatro correções saídas da validação da P3 (2026-09-14). **A fronteira `ApiPhysicallySaved` do diário exige agora gravação confirmada**: um Apply com a etapa de API Object desmarcada registrava a fronteira assim mesmo, com `ApiSaveCount=0` e sem nenhum item de API no inventário — e é justamente a fronteira que impede a recuperação de repetir o Save do API Object. **`composite.apiGuid` passou a ser o GUID do API Object**, e não o do próprio objeto: os writers de Business Component e de List gravavam `procedure.Guid`, fazendo cada Procedure apontar para si mesma no inventário durável — identidade composta é o que autoriza exclusão na etapa de remoção. **Identificar o objeto principal deixou de declarar persistência**: `SetMainObject` preenchia `PersistedMainObject`, e por isso operações bloqueadas antes da primeira gravação relatavam um objeto persistido que ninguém gravou. **E o construtor do relatório deixou de repor esse valor por fallback** para o objeto apenas identificado — a quarta correção, que só apareceu na revalidação porque a terceira, sozinha, não bastava. Gate novo `tests.journalFrontierSentinel` (verificado por mutação) e casos novos em `tests.applicationFinalReport`. Revalidado na IDE em 2026-09-14: o inventário passou a trazer o GUID do API Object nos cinco itens, a fronteira continua sendo registrada quando há gravação real, e o Apply sem API Object passou a fazer três checkpoints em vez de quatro. A revalidação mostrou que a correção do relatório estava incompleta — o construtor de `ApiPlanApplicationFinalReport` repunha o valor por fallback para o objeto identificado, e o gate testava o collector em vez do `Build()`; o fallback foi removido e o caso de teste passou a exercitar o relatório construído. **Tudo validado na IDE em 2026-09-14**: o Apply sem API Object fecha com três checkpoints, sem fronteira registrada e com `PersistedMainObject` vazio. Evidência: `Docs/Implementation/2026-09-14-S-B111-F3-P3-GATE-ESTENDIDO.md`, seções 6.6, 6.7, 9, 10 e 11.
- `S-B111 F3` — etapa P3 (2026-09-14): o bloqueio do diário deixou de ser uma frase. O gate estendido da seção 4.2 do plano da F3 passa a classificar por que uma operação não pode começar, com a precedência fechada da decisão 24 — `JournalUnavailable` sobre `DurabilityUnknown`, sobre `GateBlocked`, e `PreconditionFailed` fora de `GateBlocked` —, `reasonCode` estável, a pré-condição que reprovou e contexto estruturado (`operationId`, `journalFileId`, fase, estágio, durabilidade e o `blockReason` já persistido). Duas lacunas de validação foram fechadas: um File de diário **de outra KB** era aceito como governante desta, e `Prepared` era indistinguível de um envelope ativo — agora tem razão própria e o ponto de autorização que a P6 vai preencher. O enum `blockReason` recebeu **`UserAborted`**: um aborto do usuário era gravado como `StageFailed`, valor que a decisão 24 reserva a falha comunicada por `NoteStageFailed`. É mudança incompatível de schema, feita agora porque o V1 nunca saiu num release. Gate novo `tests.operationJournalGate`. **Validado na IDE em 2026-09-14** na `Escola` da `wsEducacaoSpTeste`, nos cinco cenários do roteiro: aborto gravando `UserAborted`, Apply e Sync recusados com o diagnóstico classificado e sem tocar no envelope, diário adulterado para outra KB e colisão externa no nome do File. Os dois últimos são caminhos que passavam antes da P3; o do diário de outra KB provou em campo a precedência da decisão 24, porque o mesmo envelope `Partial` que produzia `GateBlocked` passou a ser recusado como `JournalUnavailable`. A bateria expôs três defeitos anteriores à P3, todos corrigidos e validados na entrada acima: `composite.apiGuid` gravado com o GUID do próprio objeto pelos writers de Business Component e List, `SetMainObject` declarando persistência ao apenas identificar o API Object, e a fronteira `ApiPhysicallySaved` sendo registrada sem gravação do API Object. Evidência: `Docs/Implementation/2026-09-14-S-B111-F3-P3-GATE-ESTENDIDO.md`, seções 6 e 9.
- `S-B111 F3` — custo do diário medido na KB grande (2026-09-14): **182 ms** para os quatro checkpoints de um Apply de 44,2 s na `Empresa` da `FabricaBrasil18Test`, ou 0,4% da operação, contra os ~4,4 s previstos no orçamento da seção 4.4 do plano. O aborto anterior, com três checkpoints, custou 125 ms. O plano da F3 recebeu nota de remissão nas seções 2.3 e 4.4: a medição de ~1,1 s por gravação de File na KB grande, de 2026-09-04, não se confirmou em campo. A política de checkpoints não muda — é contrato —, mas o risco de regressão de tempo do Modo A não se materializou. Evidência: seção 6.6 de `Docs/Implementation/2026-09-14-S-B111-F3-P2-DIARIO-NA-KB.md`.
...
- `S-B111 F3` — recibos e inventário no diário (2026-09-14): cada checkpoint leva para o envelope os `PersistenceReceipt` da F2 e um inventário derivado deles, com identidade, ação, estado físico e confirmação por alvo. Validado na IDE na `Escola`: 12 recibos duráveis batendo com o relatório, 7 alvos, envelope de 7616 bytes e **305 ms** de custo — praticamente o mesmo de um envelope de 1 KB. As sequências persistidas provam a ordem da F1 por dado: Procedures (1–5), Business Component (6–9), List (10), API Object (11) e metadata (12). Três defeitos encontrados no caminho e corrigidos: o diário não pode mais derrubar a operação que observa — uma recusa do schema vira bloqueio visível com o relatório preservado —, `composite.apiGuid` deixou de ser exigido em gravação (a identidade histórica completa é exigência de quem autoriza exclusão) e a chave do inventário passou a ser a mesma do validador. Gate novo `tests.operationJournalReceipts`. Evidência: seções 6.1 a 6.4 de `Docs/Implementation/2026-09-14-S-B111-F3-P2-DIARIO-NA-KB.md`.
- `S-B111 F3` — etapa P2 (2026-09-14): o diário durável passa a existir na KB. Apply e Sincronizar registram a intenção no File único `GxOpenApiBuilder_OperationJournal` **antes** de qualquer gravação de negócio, em duas fases confirmadas (`Prepared` e `Active`), e fecham o diário na conclusão, na falha de etapa e no aborto do usuário. Cada checkpoint é um `Save()` seguido de releitura por `FileId` e confirmação por bytes e hash; a durabilidade do diário é registrada separadamente dos recibos da F2. Um envelope anterior não terminal **bloqueia** a operação seguinte, em vez de ser sobrescrito — e, até a etapa de recuperação explícita, a única saída é apagar o File do diário à mão. Custo previsto: quatro gravações por operação, ~0,5 s na KB pequena e ~4,4 s na grande. Remove e Recovery já existem na máquina de checkpoints, mas ainda não são acionados. Gate novo `tests.operationJournalCheckpoints`. A entrega saiu **sem validação na IDE**, e a validação veio na mesma data, nos quatro fluxos que o diário cobre — Apply de criação, Apply de reencontro, Apply abortado com bloqueio da operação seguinte e Sync com delta —, em `wsEducacaoSpTeste` e `FabricaBrasil18Test`, com três defeitos encontrados e corrigidos no caminho; o custo medido ficou uma ordem de grandeza abaixo do orçamento. Evidência: `Docs/Implementation/2026-09-14-S-B111-F3-P2-DIARIO-NA-KB.md`, seções 6.1 a 6.8.
- `S-B111 F3` — etapas P0 e P1 (2026-09-14), offline e sem tocar em fluxo: a metadata de negócio passou a ser emitida como `GOAB_API_METADATA_B060_V3`, com `ownership.applicationId` no payload e, por consequência, no fingerprint; a leitura continua tolerando V1 e V2, e metadata legada **não** é regravada só para preencher o campo novo. Entrou também o schema V1 do diário durável `GxOpenApiBuilder_OperationJournal` — modelo com enums fechados, serializer canônico determinístico (ordem do schema, nulos presentes, arrays vazios, GUID `D` minúsculo, UTC com milissegundos, sem espaços supérfluos), `snapshotHash` SHA-256 desse material e validador que recusa o envelope inválido antes de qualquer `File.Save()`. Nenhum diário é gravado em KB nenhuma ainda: isso é a P2. Gate novo `tests.operationJournalSchema`. Evidência: `Docs/Implementation/2026-09-14-S-B111-F3-P0-P1-IMPLEMENTACAO-OFFLINE.md`.
...
- `Remover API gerada` não depende mais de a lista de SDTs vir na ordem de dependência. O que a IDE recusa por estar referenciado volta para a fila e é tentado na passada seguinte; quando uma passada inteira não faz progresso, a remoção para e lista os pendentes com o motivo de cada um. Antes, uma ordem imperfeita interrompia a operação no meio, com objetos já apagados. Medido em 2026-09-06 na `Teste` de `wsEducacaoSpTeste`: 17 apagados na primeira passada, 1 adiado, resolvido na segunda; `Removidos=24`, `Bloqueados=0`.
...
- `S-B111 F3` — etapa P2 **validada na IDE** (2026-09-14). Na KB pequena `wsEducacaoSpTeste`, Transaction `Escola`: Apply de criação nova com quatro checkpoints confirmados (314 ms) e o cruzamento `journal.applicationId` = `metadata.ownership.applicationId`, que fecha o critério de aceite 8; Apply de reencontro com 12 recibos duráveis e 7 alvos (305 ms), cujas sequências provam a ordem da F1 — API Object na sequência 11, depois de todos os consumidores. Na KB grande `FabricaBrasil18Test`, Transaction `Empresa`: aborto terminando em `Partial`/`NotStarted` com `blockReason=StageFailed` e **bloqueio** do Apply seguinte antes da primeira gravação; Apply completo em 182 ms de diário sobre 44,2 s de operação; e reuso do File confirmado (`Created=False`, mesmo `FileId`, `operationId` e `applicationId` novos). A comparação entre dois Applies seguidos provou em campo a promoção V3: mesma metadata, mesmo `PlannedContractHash`, mesmo tamanho, `Sha256` diferente — só o `ownership.applicationId` mudou. O `Sincronizar` com delta fechou o ciclo na `Escola` (`EscolaEndereco` 70→71): `operationKind='Sync'`, File reutilizado, quatro checkpoints e 156 ms. Evidência: seções 6.1 a 6.8 de `Docs/Implementation/2026-09-14-S-B111-F3-P2-DIARIO-NA-KB.md`.
