...
- `B096` (Sprint 9 / Fase 2): plano de SDTs hierárquicos por contrato quando `ApiPlan.Levels` tem filhos; naming `sdt<Tx>_API_<Papel>_<caminho>` com desambiguação por `LevelOrder` e encurtamento de objeto a 128 quando o nome completo estoura 128 ou colide (folha ≤32 escolhe entre reusar a folha e o hash; membros sem teto nesta fase); marcador `<Subnível>Replace` no Update; pós-ordem em `OwnSdts`; `ListResponse.Items` permanece coleção de `Response`; ouro em `Tests/SdtHierarchicalPlan/Baselines/` e gate `tests.sdtHierarchicalPlan`. A trava `Test-OpenApiClientContractValidity.ps1` passa a ler também `ApiPlanSdtHierarchicalNaming.cs` para `_API_CreateRequest_`, `_API_UpdateRequest_` e `_API_Response_` (sem `_API_ListResponse_Item`). Wizard flat e `ListResponse_Item` fora deste recorte. Evidência: `Docs/Implementation/2026-08-26-B096-SDTS-HIERARQUICOS.md`.
...
- Checker e `AGENTS.md`: `currentFront`/`manualRequired` documentados como exclusivos de spike `B000`–`B006`; lista vazia com próxima ação `B007+` (ex. `B098`) é o contrato, não falso verde, e não substitui a revisão semântica.
