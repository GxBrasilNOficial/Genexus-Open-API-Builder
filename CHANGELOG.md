...
- `S-B111 F1` (2026-09-08): Apply/Sync preparam um único API Object transitório quando BC ou List participa; Procedures de consumidor são salvas antes, e somente o writer final grava o API Object uma vez. O contexto carrega GUID planejado, seleção, identidade da aplicação e snapshot pré-mutação; o relatório separa nome planejado de objeto persistido e conta os Saves físicos. A implementação recebeu validação manual parcial na IDE em 2026-09-10; os caminhos positivos de Sync sem BC/List e somente BC e o guard de divergência manual foram validados, mas ainda faltam somente List e o guard de BC sem habilitação. F2 e F3 permanecem planejadas.
...
- `S-B111 F1` (2026-09-10): validação manual parcial na IDE da matriz positiva do Wizard, da guarda de API já REST-completa, do Sync `BC + List` com delta `NotaFiscalObs2` 40→41, do Sync sem BC/List com `LaudoObs` e do Sync somente BC na `Carga` com `CargaObservacao2` `VarChar(40)`; a Output confirmou consumidores antes do API Object, um único `API.Save()` nos caminhos positivos e bloqueio de divergência manual com `ApiSaveCount=0`; a restauração produziu diff zero. `Build All` passou em `NETPostgreSQL155` e `NETFrameworkSQLServer004` antes do último delta da `Carga`. Permanecem sem evidência separada somente List e BC sem habilitação. Evidência: `Docs/Implementation/2026-09-10-S-B111-F1-ACEITE-IDE.md` e `Docs/Implementation/2026-09-10-S-B111-F1-RECONCILIACAO-EVIDENCIA.md`.
...
- `B108` (plano aprovado 2026-08-31; estacionado desde 2026-09-05): preferências só na criação; reencontro espelha KB; desmarcar confirma e rebaixa/remove no Apply (Delete some com BC). Plano: `Docs/Implementation/2026-08-31-B108-PLANO-PREFERENCIAS-E-RETRACAO.md`. A próxima ação única é fechar os dois cenários restantes de Sync da F1 antes de preparar a avaliação da F2 da sprint `S-B111`; ver o checkpoint, o documento 06 e a reconciliação `Docs/Implementation/2026-09-10-S-B111-F1-RECONCILIACAO-EVIDENCIA.md`.
- Escrita parcial do BC (`ApiPlanBusinessComponentWriter.Apply` grava API Object antes das Procedures): drift API↔metadata B067 se o Sync/Apply aborta no meio. Virou a sprint **`S-B111`**, com três planos de fase — F1 ordem e writer final único, F2 seam e recibos, F3 durabilidade e remoção —. A F1 foi implementada e recebeu validação manual parcial na IDE em 2026-09-10; a matriz manual de Sync ainda precisa ser fechada antes da avaliação da F2. F2 e F3 continuam planejadas. A orientação anterior de recuperação por "Remover + Wizard" **não vale quando a metadata está ausente**: nesse estado, `Remover` e `Sincronizar` bloqueiam e o `Wizard` degrada o plano; a saída é a recuperação de metadata órfã acima, ou limpeza manual (`B115`). Planos em `Docs/Implementation/2026-09-04-B111-F1-…`, `…-F2-…` e `…-F3-…`; medições que os sustentam em `Docs/Implementation/2026-09-04-B111-SONDAS-IDENTIDADE-E-DIARIO.md`.
