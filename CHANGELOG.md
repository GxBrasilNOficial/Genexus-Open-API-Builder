...
- `B096` (Sprint 9 / Fase 2): plano de SDTs hierárquicos por contrato quando `ApiPlan.Levels` tem filhos; naming `sdt<Tx>_API_<Papel>_<caminho>` com desambiguação por `LevelOrder` e encurtamento a 128 via hash; marcador `<Subnível>Replace` no Update; pós-ordem em `OwnSdts`; `ListResponse.Items` permanece coleção de `Response`; ouro em `Tests/SdtHierarchicalPlan/Baselines/` e gate `tests.sdtHierarchicalPlan`. Wizard flat e `ListResponse_Item` fora deste recorte. Evidência: `Docs/Implementation/2026-08-26-B096-SDTS-HIERARQUICOS.md`.
