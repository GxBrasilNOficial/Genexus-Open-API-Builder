...
- `S-B111 F3` — **diário durável de operação** (2026-09-14): a extensão passou a registrar na própria KB o que pretende fazer, antes de fazer. Há um File único por KB, `GxOpenApiBuilder_OperationJournal`, no Root Module, gravado por Apply e Sincronizar **antes** de qualquer objeto de negócio, em duas fases confirmadas (`Prepared` e `Active`); cada checkpoint é um `Save()` seguido de releitura por `FileId` e confirmação por bytes e hash. O envelope tem forma canônica determinística — ordem do schema, nulos presentes, arrays vazios, GUID minúsculo, UTC com milissegundos — com `snapshotHash` SHA-256 desse material, e um envelope inválido é recusado antes do `File.Save()`. Ele carrega recibos e inventário por alvo — identidade, ação, estado físico e confirmação —, de modo que a ordem de gravação fica provada por dado, e não por linha de log. Um envelope anterior não terminal **bloqueia** a operação seguinte em vez de ser sobrescrito, com diagnóstico classificado: `JournalUnavailable` sobre `DurabilityUnknown`, sobre `GateBlocked`, e `PreconditionFailed` fora de `GateBlocked`, cada um com `reasonCode` estável, a pré-condição que reprovou e contexto estruturado. O gate recusa também um diário de outra KB e um File homônimo que não seja do gerador. O motivo do bloqueio é persistido em `blockReason`, que inclui `UserAborted` para a interrupção deliberada — valor novo no enum, mudança incompatível de schema feita enquanto o V1 não saiu num release. A metadata de negócio passou a `GOAB_API_METADATA_B060_V3`, com `ownership.applicationId` no payload e no fingerprint; a leitura continua tolerando V1 e V2, e metadata legada **não** é regravada só para preencher o campo novo. O diário é instrumento de diagnóstico e nunca derruba a operação que observa: uma recusa do próprio envelope vira bloqueio visível, com o relatório final preservado. O `Remover API gerada` passou a registrar a intenção — o inventário completo dos alvos, com a identidade de cada um — **antes do primeiro `Delete()`**, e a excluir por uma fila única de todos os tipos removíveis, em passadas, com orçamento `max(1, itens da fila)`: um alvo que a IDE recusa por dependência volta para a fila e sai na passada seguinte, um resultado indeterminado bloqueia sem retry, e o envelope termina em `Removed`, `Partial` ou `OutcomeUnknown` com o motivo do enum fechado. Uma remoção interrompida passa a ter o que foi previsto e o que saiu registrados de forma durável. Existe agora o comando **`Recuperar operação interrompida`**, nas duas superfícies de menu e nos três idiomas: ele lê o diário, relê cada alvo do inventário por identidade e oferece **uma** ação, sempre com confirmação — abandonar um envelope que foi preparado e nunca gravou nada, fechar o registro de uma remoção que já terminou, ou retomar a fila de uma remoção interrompida no mesmo envelope, com o inventário durável em vez da metadata, que pode já ter sido apagada. Quando a reconciliação não é determinística, ele não age: publica o diagnóstico e devolve a decisão. A autorização da execução é vinculada ao snapshot exato que foi lido — identidade, `FileId`, `updatedUtc` e hash canônico —, e qualquer alteração entre a leitura e a ação recusa antes de mutar. Preferência nova **`ShowRecoveryOptionProactively`**, ligada por padrão, oferece a recuperação quando o diário bloqueia Apply, Sync ou Remover. Quando a operação parou no meio sem que a continuação seja possível, a recuperação oferece **encerrar o registro**: o envelope recebe um estágio terminal próprio, `Discarded`, com a disposição de quem encerrou, e **nenhum objeto é apagado** — recibos e inventário do que ficou pela metade permanecem gravados. A Knowledge Base é liberada, e as duas saídas seguintes continuam sendo reaplicar pelo Wizard sobre o estado atual ou remover a API gerada. Um envelope interrompido **antes de gravar qualquer objeto** — o aborto mais comum, porque quem desiste desiste cedo — tem texto próprio: dizer que ele «gravou objetos e parou no meio» seria falso, e a Knowledge Base está exatamente como estava antes da operação. O encerramento recusa três situações: resultado indeterminado, envelope apenas preparado (que é abandono, não encerramento) e durabilidade não confirmada. A confirmação acontece num diálogo próprio, com largura de leitura e o inventário em bloco rolável — uma linha por objeto, com o que estava previsto e o que a Knowledge Base mostra agora. **Exercido na IDE em 2026-09-14 e 15**, em cinco cenários: remoção completa pela fila nova, alvo previsto ausente antes da exclusão, encerramento do registro de uma remoção interrompida, devolução da Knowledge Base ao estado normal e — o caso que motivou o encerramento — um Apply abortado no meio, com bloqueio da operação seguinte, oferta da recuperação, encerramento do registro e Apply normal depois. As etapas restantes da validação — envelope preparado, retomada de remoção e API legado — continuam abertas. Antes disso, a única saída de um envelope interrompido era apagar o File do diário à mão, o que apagava junto a prova do que acontecera. **Limitação declarada:** a continuação de um `Apply` ou `Sync` interrompido **não** existe — o envelope guarda identidade, hash de contrato e flags, não o contrato, e retomar o pipeline a partir disso seria inventar um plano. As etapas P4 a P7 foram implementadas **offline** e ainda não foram exercidas na IDE. Custo medido do diário: 0,4% de um Apply de 44,2 s na KB grande. Gates novos `tests.operationJournalSchema`, `tests.operationJournalCheckpoints`, `tests.operationJournalReceipts`, `tests.operationJournalGate`, `tests.removalQueue` e `tests.operationJournalRecovery`. Evidência: `Docs/Implementation/2026-09-14-S-B111-F3-P0-P1-IMPLEMENTACAO-OFFLINE.md`, `...-F3-P2-DIARIO-NA-KB.md`, `...-F3-P3-GATE-ESTENDIDO.md` e `...-F3-P4-P7-IMPLEMENTACAO-OFFLINE.md`.
- Janela do Wizard 20% mais larga (1800 px) e, em qualquer diálogo da extensão, redução automática quando o monitor for menor que a janela — antes ela só era centrada, e uma janela maior que a área útil perdia bordas e botões fora da tela. O resumo e os avisos de escala quebravam linha cedo demais.
- `S-B111 F1` (2026-09-08): Apply/Sync preparam um único API Object transitório quando BC ou List participa; Procedures de consumidor são salvas antes, e somente o writer final grava o API Object uma vez. O contexto carrega GUID planejado, seleção, identidade da aplicação e snapshot pré-mutação; o relatório separa nome planejado de objeto persistido e conta os Saves físicos. A implementação recebeu validação manual para encerramento, com a exceção explícita do `B121`, em 2026-09-10/11; os caminhos positivos de Sync sem BC/List e somente BC, o guard de divergência manual e o guard B055 de BC sem habilitação foram validados, enquanto o perfil Sync somente List permanece não comprovado isoladamente. ~~F2 e F3 permanecem planejadas.~~ Superado: a F2 foi encerrada em 2026-09-13 e a F3 está em andamento — P0, P1 e P2 entregues, a última validada na IDE em 2026-09-14.
- Opção explícita, desligada por padrão, para recuperar o File de metadata de uma API própria que ficou órfã após aplicação parcial. É oferecida **ao abrir o Wizard**, antes do diálogo — sem a metadata o Wizard abre bloqueado, e uma oferta feita depois nunca seria alcançada. Valida posse do API Object por Description e Service Source, e não altera API Object, Procedures ou SDTs. Ela cria o File que falta e também **regrava um File que ficou apontando para um API Object removido** — o que acontece quando a API é removida e gerada de novo, e antes deixava o Wizard travado sem saída pela ferramenta. **O que ela devolve é a capacidade de remover a API gerada, não o contrato original:** a metadata recuperada leva o inventário dos objetos encontrados na KB, e paginação, ordenação, campos obrigatórios e estrutura hierárquica não são recuperáveis — existiam só na metadata perdida. Por isso ela sai marcada como intenção importada, com `Remover API gerada` liberado e **`Sincronizar` bloqueado** até uma aplicação completa reescrever a metadata. Desenho e pontos em aberto: seção 13 de `Docs/Implementation/2026-09-04-B111-F3-PLANO-DURABILIDADE-E-REMOCAO.md`.
...
- Instrumentação de diagnóstico das frentes `B109`, `B111` e `B115`: a captura B109/B111 não altera o que é gravado na KB. Desde o fechamento da F2, B109 só publica o dump técnico quando houver mutação durante `Pump` ou falha de `Save`; os recibos B111 são consolidados no relatório e detalhados apenas em anomalia; B115 permanece para visibilidade e recuperação de metadata. Os critérios de remoção das sondas ainda ativas estão no checkpoint `Docs/STATUS_ATUAL_E_PROXIMO_PASSO.md`.
...
- **A janela de progresso ficava viva atrás do relatório final**, com o botão `Abortar` ativo numa operação que já havia terminado. Os comandos mostram o relatório dentro do escopo da janela, e ela só era fechada quando o método acabava — depois do diálogo. A janela passou a sair de cena no início do relatório, em todos os caminhos: sucesso, bloqueio, falha de etapa e aborto. **Presente na `0.1.0-alpha.7`**: o escopo de progresso entrou com o `B082` (commit ancestral da tag), e a correção vale para quem usa aquela versão. Ficou visível em 2026-09-14, quando o bloqueio pelo diário passou a somar um segundo diálogo por cima.
- `S-B111 F3` — oito correções internas ao diário e ao relatório final (2026-09-14). **Nenhuma delas saiu em release publicado**: todas atingem mecanismo introduzido depois da `0.1.0-alpha.7` e que estreia neste bloco. Ficam registradas porque testadores usam builds locais entre cortes, e porque o que quebrou no caminho explica por que o desenho é o que é. **Quatro foram expostas pela validação do gate**, duas delas no mesmo caminho do relatório: ele chamava de persistido um objeto que ninguém gravou — identificar o API Object na KB — o que alimenta o botão «Abrir objeto principal» — preenchia `PersistedMainObject`, e o construtor do relatório ainda repunha o valor por fallback quando o collector o deixava vazio. **A fronteira `ApiPhysicallySaved` era registrada sem gravação**: um Apply com a etapa de API Object desmarcada a registrava com `ApiSaveCount=0` e sem nenhum item de API no inventário — e é justamente a fronteira que impede a recuperação de repetir o Save do API Object. **`composite.apiGuid` carregava o GUID do próprio objeto**: os writers de Business Component e de List gravavam `procedure.Guid`, fazendo cada Procedure apontar para si mesma no inventário durável, quando identidade composta é o que autoriza exclusão na etapa de remoção. Gate novo `tests.journalFrontierSentinel` e casos novos em `tests.applicationFinalReport`, ambos verificados por mutação. **A oitava veio da validação na IDE:** o File do diário nascia **sem módulo atribuído** — a IDE aceita, mas o objeto ficava fora da organização que o resto da extensão segue, e uma extensão de terceiros instalada na mesma IDE avisava. Como o diário pertence à Knowledge Base, e não a uma Transaction, ele passou a nascer no `Root Module`; a atribuição é idempotente e corrige também um diário criado antes, na primeira gravação seguinte. A linha de abertura do diário passou a publicar o módulo, para que isso não dependa de inspeção manual. O aviso daquela extensão **continua aparecendo**, e está certo assim: ela cobra a ausência de **Folder**, não de módulo, e Files desta extensão ficam no módulo por decisão de 2026-07-28 — a metadata de negócio, que sempre teve módulo, aparece na mesma lista. **Três foram corrigidas durante a validação do próprio diário**, antes do gate existir: uma recusa do envelope pelo schema derrubava a operação que ele observa, em vez de bloquear a si mesmo com o relatório preservado; `composite.apiGuid` era exigido em gravação, quando identidade histórica completa é exigência de quem autoriza exclusão; e a chave de agregação do inventário divergia da chave de unicidade do validador, transformando dois recibos do mesmo alvo em itens que colidiam. Evidência: `Docs/Implementation/2026-09-14-S-B111-F3-P3-GATE-ESTENDIDO.md`, seções 6.6, 6.7, 9 e 10, e `...-F3-P2-DIARIO-NA-KB.md`, seção 6.3.
- **O diálogo de `Recuperar operação interrompida` falava português em qualquer KB.** Os rótulos do comando e das confirmações já estavam nos três idiomas desde a P7, mas o texto que sustenta a decisão — o bloqueio, o resumo do que aconteceu, o inventário linha a linha e as recusas do executor — nasce no rehydrator e no executor, que são SDK-simples e não conhecem idioma. Trinta frases entraram no catálogo de saída em espanhol e inglês, e a lista de detalhes do diálogo passou a ser traduzida como o resumo já era. Nenhuma saiu em release publicado: o comando estreia neste bloco. Ficou visível na bateria da IDE de 2026-09-14 e foi fechada em 2026-09-15.
- `Remover API gerada` não depende mais de a lista de SDTs vir na ordem de dependência. O que a IDE recusa por estar referenciado volta para a fila e é tentado na passada seguinte. Antes, uma ordem imperfeita interrompia a operação no meio, com objetos já apagados. Medido em 2026-09-06 na `Teste` de `wsEducacaoSpTeste`: 17 apagados na primeira passada, 1 adiado, resolvido na segunda; `Removidos=24`, `Bloqueados=0`. Desde 2026-09-14 essa resiliência deixou de ser um laço exclusivo de SDTs e passou a ser a fila única da F3, que abrange também API Object, Procedures, metadata e Folder, com orçamento de passadas declarado e checkpoint durável ao fim de cada uma.
- Remoção interrompida deixa de informar `Removidos: (nenhum)` quando já apagou objetos. O relatório final passa a listar o que saiu da KB nos dois caminhos de interrupção — aborto pelo usuário e falha —, com aviso de que a API ficou incompleta.
...
- **As mensagens de bloqueio por API Object divergente passaram a dizer o que fazer.** Quando a metadata registra um API Object que não é o que está na Knowledge Base — porque ele foi apagado ou trocado fora da ferramenta —, três recusas corretas se acumulavam sem explicação: o Wizard travava nomeando um campo de JSON, o `Remover API gerada` parava sem saída e a recuperação de metadata órfã não se oferecia. Agora as três dizem que o caminho é apagar **o API Object e o File de metadata** — os dois, porque um sem o outro apenas troca um bloqueio pelo outro — e o que se perde ao fazer isso: paginação, ordenação e campos obrigatórios voltam aos padrões das preferências, porque só existiam na metadata apagada. O mesmo vale quando a metadata está ausente e o API Object presente. Os outros descompassos de posse continuam sem sugestão de propósito: Description alheia, integridade divergente e Service Source fora do contrato não se resolvem apagando nada. Nos três idiomas.
- **O bloco técnico do bloqueio mostra só o que foi medido.** O diagnóstico de posse do API Object publicava trinta e três linhas fixas — onze vazias e cinco derivadas de uma medição que não tinha acontecido, porque sem fingerprint gravado não há o que comparar. Os blocos de fingerprint e de baseline passaram a abrir apenas quando existem, campo textual vazio deixou de virar linha, e a cláusula que falhou não se repete: ela já encabeça a causa.
- **Remover duas vezes a mesma API deixou de ser silenciosamente aceito.** Um alvo previsto que já não está na Knowledge Base antes da exclusão encerra a operação como remoção parcial, com o motivo registrado, em vez de ser tratado como sucesso: quem apagou aquele objeto não foi esta operação, e chamar isso de sucesso é o que fazia um relatório dizer «Removidos: nenhum» com objetos apagados. A saída, quando o estado for realmente o de uma remoção já concluída, é o comando `Recuperar operação interrompida`, que relê cada alvo e fecha o registro. Comportamento alterado em 2026-09-14 pela etapa P4 da `S-B111 F3`; vale para quem vinha da `0.1.0-alpha.7`, onde a remoção era idempotente por omissão. Ainda **sem validação na IDE**.
- Relatório final B081 e Output normal compactados após a validação da F2: em sucesso, o diálogo apresenta writer e contagem do API Object, recibos confirmados/pendentes, totais de objetos, duração e avisos; não lista GUIDs, recibos ou objetos individuais. Recibos não confirmados, falhas de etapa e bloqueios continuam detalhados. A Output deixa de repetir cada `Save` B111 e cada item B081; a sonda B109 só publica fronteiras e hashes quando detectar mutação durante `Pump` ou falha de `Save`; telemetria detalhada B082 fica restrita à depuração. Smoke IDE em `Teste` (2026-09-13): 12 recibos confirmados, 0 pendências, 1 `API.Save` pelo writer `List`, 28 atualizações e 2 avisos.
...
- `S-B111 F3` — o diário exercido na IDE (2026-09-14), em duas KBs. **Os quatro fluxos que ele cobre**, em `wsEducacaoSpTeste` e `FabricaBrasil18Test`: Apply de criação nova, com o cruzamento `journal.applicationId` = `metadata.ownership.applicationId` que fecha o critério de aceite 8; Apply de reencontro, com 12 recibos duráveis cujas sequências provam a ordem da F1 — API Object na sequência 11, depois de todos os consumidores; Apply abortado, terminando em `Partial` com `blockReason=UserAborted` e **bloqueio** do Apply seguinte antes da primeira gravação; e `Sincronizar` com delta. Custos: 314 ms, 305 ms e 156 ms na KB pequena, 182 ms na grande — contra os ~4,4 s do orçamento, que o plano passou a registrar como superestimado. **Os seis cenários do gate**, na `Escola`: os cinco do roteiro — aborto, Apply e Sync recusados sem tocar no envelope, diário adulterado para outra KB e colisão externa no nome do File — e um sexto, nascido da bateria, que encontrou a fronteira registrada sem gravação. O caso do diário de outra KB provou a precedência em campo: o mesmo envelope `Partial` que produzia `GateBlocked` passou a ser recusado como `JournalUnavailable`. A comparação entre dois Applies seguidos provou também a promoção V3 da metadata — mesmo contrato, mesmo tamanho, `Sha256` diferente, porque só o `ownership.applicationId` mudou. Evidência: seções 6.1 a 6.8 de `...-F3-P2-DIARIO-NA-KB.md` e seções 6 e 10 de `...-F3-P3-GATE-ESTENDIDO.md`.
- `S-B111 F1` (2026-09-10/11): validação manual na IDE, para encerramento da F1 com exceção explícita do `B121`, da matriz positiva do Wizard, da guarda de API já REST-completa, do Sync `BC + List` com delta `NotaFiscalObs2` 40→41, do Sync sem BC/List com `LaudoObs` e do Sync somente BC na `Carga` com `CargaObservacao2` `VarChar(40)`; a Output confirmou consumidores antes do API Object, um único `API.Save()` nos caminhos positivos e bloqueio de divergência manual com `ApiSaveCount=0`; a restauração produziu diff zero. A tentativa de Sync somente List na `Contrato` executou BC (`Get/Create/Update`) antes de List, apesar de não haver opção de BC no diálogo; `FinalWriter='List'`, `ApiSaveCount=1` e `Bloqueados=0`, mas o perfil isolado não foi comprovado e o fato foi registrado no `B121`. O reteste de BC sem habilitação bloqueou no preflight com `B055`, `ApiSaveAttempted=False`, `ApiSaveCount=0`, `Criados/Atualizados/Removidos=0` e `Bloqueados=1`, sem alteração na KB. `Build All` passou em `NETPostgreSQL155` e `NETFrameworkSQLServer004` antes do último delta da `Carga`. Permanece sem comprovação isolada somente List; `B121` fica fora da sprint. Evidência: `Docs/Implementation/2026-09-10-S-B111-F1-ACEITE-IDE.md`, `Docs/Implementation/2026-09-10-S-B111-F1-RECONCILIACAO-EVIDENCIA.md` e `Docs/Implementation/2026-09-10-B121-SYNC-SELECAO-BC-LIST.md`.
- `S-B111 F2` (2026-09-12/13): implementação do seam de persistência e recibos aceita na IDE com cenário positivo do Wizard, habilitação diferida de Business Component, cancelamento cooperativo, recomposição da `Produto`, criação em escala da `Empresa` e falhas controladas. A `Teste` descartável comprovou retorno sem mutação antes de `SDT.Save`, confirmação divergente após `API.Save` e falha antes de `Procedure.Delete`, seguida de limpeza idempotente. A F2 foi encerrada; F3 permanece separada para durabilidade, reconciliação e retry. Evidência: `Docs/Implementation/2026-09-12-S-B111-F2-ACEITE-IDE.md`.
