...
- `S-B111 F1` (2026-09-08): Apply/Sync preparam um único API Object transitório quando BC ou List participa; Procedures de consumidor são salvas antes, e somente o writer final grava o API Object uma vez. O contexto carrega GUID planejado, seleção, identidade da aplicação e snapshot pré-mutação; o relatório separa nome planejado de objeto persistido e conta os Saves físicos. A implementação recebeu validação manual parcial na IDE em 2026-09-10; o caminho positivo de Sync sem BC/List e o guard de divergência manual foram validados, mas a matriz manual de Sync ainda não foi fechada. F2 e F3 permanecem planejadas.
...
- `S-B111 F1` (2026-09-10): validação manual parcial na IDE da matriz positiva do Wizard, da guarda de API já REST-completa, do Sync `BC + List` com delta `NotaFiscalObs2` 40→41 e do Sync sem BC/List com `LaudoObs`; a Output confirmou consumidores antes do API Object, um único `API.Save()` no caminho positivo e bloqueio de divergência manual com `ApiSaveCount=0`; a restauração produziu diff zero. `Build All` passou em `NETPostgreSQL155` e `NETFrameworkSQLServer004`. Permanecem sem evidência separada os perfis Sync somente BC, somente List e BC sem habilitação. Evidência: `Docs/Implementation/2026-09-10-S-B111-F1-ACEITE-IDE.md` e `Docs/Implementation/2026-09-10-S-B111-F1-RECONCILIACAO-EVIDENCIA.md`.
...
- `B108` (plano aprovado 2026-08-31; estacionado desde 2026-09-05): preferências só na criação; reencontro espelha KB; desmarcar confirma e rebaixa/remove no Apply (Delete some com BC). Plano: `Docs/Implementation/2026-08-31-B108-PLANO-PREFERENCIAS-E-RETRACAO.md`. A próxima ação única é fechar os três cenários restantes de Sync da F1 antes de preparar a avaliação da F2 da sprint `S-B111`; ver o checkpoint, o documento 06 e a reconciliação `Docs/Implementation/2026-09-10-S-B111-F1-RECONCILIACAO-EVIDENCIA.md`.
