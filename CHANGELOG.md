...
- Documentação B098: residual `count(HeaderId)` na fixture `InheritedPrimaryKey` / PK composta herdada registrado no checkpoint e na evidência — a correção de `ResolveAggregateAttributeName` passa a ser o primeiro item da Fase 5-A (`B099v`); o gatilho anterior ("antes do smoke IDE multinível") venceu em 2026-08-26 sem ser cumprido, e o ouro atual congela o emissor.
