...
- Escrita parcial do BC (`ApiPlanBusinessComponentWriter.Apply` grava API Object antes das Procedures): drift API↔metadata B067 se o Sync/Apply aborta no meio. Virou a sprint **`S-B111`**, com três planos de fase — F1 ordem e writer final único, F2 seam e recibos, F3 durabilidade e remoção —. A F1 foi implementada e validada manualmente na IDE para encerramento, com a exceção explícita do `B121`, em 2026-09-10/11; o guard B055 de BC sem habilitação passou no preflight com zero gravações, e o perfil Sync somente List permanece não comprovado isoladamente no `B121`. A F2 e a F3 continuam planejadas. A orientação anterior de recuperação por "Remover + Wizard" **não vale quando a metadata está ausente**: nesse estado, `Remover` e `Sincronizar` bloqueiam e o `Wizard` degrada o plano; a saída é a recuperação de metadata órfã acima, ou limpeza manual (`B115`). Planos em `Docs/Implementation/2026-09-04-B111-F1-…`, `…-F2-…` e `…-F3-…`; medições que os sustentam em `Docs/Implementation/2026-09-04-B111-SONDAS-IDENTIDADE-E-DIARIO.md`.
