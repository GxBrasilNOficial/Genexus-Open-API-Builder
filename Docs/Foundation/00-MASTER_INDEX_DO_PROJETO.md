...
**Status técnico:** Extensão em linha Alpha; Sprint 9 com `Delete` opt-in no pacote `0.1.0-alpha.6` (publicação sob autorização humana); próxima ação no [checkpoint](../STATUS_ATUAL_E_PROXIMO_PASSO.md)
**Status público:** Repositório público; releases Alpha no GitHub (último corte publicado `0.1.0-alpha.5`; pacote `0.1.0-alpha.6` preparado)
**Idioma:** Português BR
