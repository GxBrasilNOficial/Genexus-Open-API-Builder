...
| B100 | Serviço `Delete` opt-in, com confirmação consciente, `SecurityLevel` próprio e documentação pública | Alta — Sprint 9 / após a Fase 7, corte `0.1.0-alpha.6`; concluído (2026-08-30; HTTP 401/404/200 nos dois environments, 422 no Framework); pacote do corte preparado em 2026-08-31 (publicação pendente) |
...
| B108 | Preferências da KB só na criação da API; no reencontro, preservar o contrato já gravado e deixar o operador alterar o que quiser | Alta — urgente; próxima sessão de código; planejado (2026-08-31) |
...
**B108 — preferências da KB só na criação; urgente para a próxima sessão (2026-08-31).** O File `GxOpenApiBuilder_Settings` (B068) é default por KB. Hoje o Wizard aplica esses defaults também no reencontro (`ApplyPreference`, `_applyBusinessComponentWhenReady`, `ResolveApplyBusinessComponentAfterGenerationRefresh`). Observado na `NotaFiscal`: após Apply com “Completar REST via Business Component” desmarcado, a reabertura religou o checkbox porque a preferência da KB estava marcada. A regra pedida: defaults da KB apenas na **primeira geração**; no **update/reencontro**, preservar o que já está na API e na metadata (o mesmo espírito de `ResolveExistingServiceSelection` para serviços), e o operador só muda o que quiser. Escopo: todas as preferências de geração da API (SDTs, Procedures, API Object, List, BC, metadata — e o alinhamento com o que a metadata já persiste: serviços, segurança, paginação). Não misturar com o corte `0.1.0-alpha.6` até decisão humana.
