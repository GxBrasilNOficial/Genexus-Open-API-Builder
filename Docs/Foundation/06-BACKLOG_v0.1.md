...
**Atualização da S-B111 — 2026-09-07.** A documentação dos planos F1, F2 e F3 foi
consolidada com as decisões aprovadas: Modo A, diário único por KB sem histórico, ciclo
`Prepared`/`Active`/`Ready`, `OperationId` separado de `ApplicationId`, B115 dentro do
escopo gerenciado, `OutcomeUnknown`, `StillPresentAfterDelete`, fila de remoção por
passadas e comando unificado de recuperação. Nenhuma fase foi implementada, a revisão por
pares não foi encerrada e B109, B110, B112, B113 e B114 permanecem fora da S-B111.
