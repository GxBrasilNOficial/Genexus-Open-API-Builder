...
| B082 | Mostrar tempo execução / sinal de vida | Média — **Fases A+B em código** (2026-08-31); Preview do Sync como o Remover (2026-09-01). Estacionado, **não é a próxima ação**. Smoke `Empresa`: Apply ~4,2 min; Sync `PreviewMs=5089`; Remover Preview `PreviewMs=2525` (Não). Fora da linha de corte do MVP e **fora do corte `0.1.0-alpha.5`**. Registro `Docs/Implementation/2026-08-31-B082-PLANO-UX-PROGRESSO.md`. Não misturar com `B108` |
...
| B108 | Preferências da KB só na criação; no reencontro checkboxes espelham a KB; desmarcar confirma e rebaixa/remove no Apply (Delete some com BC) | Alta — planejado e aprovado (2026-08-31); implementação adiada; plano em `Docs/Implementation/2026-08-31-B108-PLANO-PREFERENCIAS-E-RETRACAO.md` |
...
`B082` fica fora da linha de corte do MVP e fora de `0.1.0-alpha.5`. Fases A+B estão em código desde 2026-08-31 (estacionado; próxima ação é `B108`). A `Empresa` em 2026-08-29 motivou o sinal de vida; Apply ~4,2 min em 2026-08-31; Preview do Sync (`5089` ms) e do Remover (`2525` ms, Não) em 2026-09-01.
