...
| B102 | Repasse do texto emitido pelo Business Component na `Message` do `422`, com `Message` em `LongVarChar`, experimento de `Messages[]` como coleção, filtro por mensagens de erro e opção de desligar por KB e por API | Alta — Sprint 9 / **primeiro item**; planejado |
...
**B102 — urgente, primeiro item da Sprint 9.** O registro de decisões estabelece, desde 2026-07-14, que a `Message` de erro é texto legível produzido pela aplicação e que a extensão não traduz mensagens do Business Component. A geração atual não cumpre essa decisão: em falha de `Save()`, responde `422` com o texto fixo `"Business rules rejected the request."` e descarta as mensagens do BC, de modo que uma rule `error` da KB nunca chega ao consumidor. O item implementa o repasse, com as decisões fechadas na revisão de plano de 2026-08-23:

- **`Message` passa a `LongVarChar`**, com truncamento explícito pela geração em cerca de 2K e reticência final, no lugar do `VarChar(256)` que cortava em silêncio. A mudança é no SDT compartilhado `sdt_API_ErrorResponse`, que atinge toda API — e por isso precisa acontecer aqui, antes da linha de base da Fase 0. Nota de coerência a registrar: o Wizard classifica `LongVarChar` como tipo tecnicamente inadequado para **atributos da Transaction** entrando no payload; `Message` é membro fixo do contrato de erro, gerado pela extensão, e a regra não se aplica a ele.
- **Experimento antes de escolher a forma do corpo.** A retirada de `Errors[]` pela `Emenda técnica — 2026-08-03` decorreu de uma tentativa com subestrutura aninhada no próprio SDT (`sdt_API_ErrorResponse.Error`), recusada pela IDE. Coleção tipada por SDT **separado** é outro mecanismo, o mesmo de `ListResponse.Items`, e nunca foi testado no corpo de erro. Se a IDE aceitar, entra o membro coleção `Messages` tipado por `sdt_API_ErrorMessage`; se recusar, as mensagens vão concatenadas por `" | "` e a recusa fica registrada como evidência. Em qualquer caso `Message` permanece top-level e preenchido.
- **Somente mensagens de erro** são repassadas; warnings do Business Component não entram no corpo do `422`.
