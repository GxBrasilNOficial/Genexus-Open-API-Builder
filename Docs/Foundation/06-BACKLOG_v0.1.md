...
| B082 | Mostrar tempo execução / sinal de vida | Média — **Fases A+B em código** (2026-08-31); Preview do Sync como o Remover (2026-09-01). Estacionado, **não é a próxima ação**. Residual P2 no registro (casca × B081 no Apply/Sync; `progress` na revalidação do Remove; `GetAll` no Remove efetivo). Smoke `Empresa`: Apply ~4,2 min; Sync `PreviewMs=5089`; Remover Preview `PreviewMs=2525` (Não). Fora da linha de corte do MVP e **fora do corte `0.1.0-alpha.5`**. Registro `Docs/Implementation/2026-08-31-B082-PLANO-UX-PROGRESSO.md`. Não misturar com `B108` |
