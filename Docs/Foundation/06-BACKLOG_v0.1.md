...
| B109 | Bug intermitente na etapa de Business Component: `Collection was modified; enumeration operation may not execute` interrompe o Apply sem cancelamento e antes de qualquer gravação | Alta — observado em 2026-09-04 na `Empresa` (44 SDTs, subníveis); não reproduziu na execução seguinte; evidência em `Docs/Implementation/2026-09-04-EVIDENCIA-IDE-DRIFT-API-OBJECT.md` §3 |
...
| B111 | API Object gravado uma única vez ao fim do pipeline: Procedures antes do API Object nos dois writers, `deferApiSave` na etapa de Business Component quando a de List também vai rodar, e trilha da ordem de gravação na Output | Alta — origem no P1 do plano `B082` (escrita parcial do BC); plano em construção em `Docs/Implementation/2026-09-04-PLANO-API-OBJECT-GRAVACAO-UNICA.md`, **em revisão por pares**, não implementado; linha de base de campo em `Docs/Implementation/2026-09-04-EVIDENCIA-IDE-DRIFT-API-OBJECT.md` |
