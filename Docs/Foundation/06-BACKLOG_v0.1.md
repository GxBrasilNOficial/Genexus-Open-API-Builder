...
**B100 — serviço `Delete`.** Frente própria, executada após a Fase 7. Contrato: `200` com a chave primária removida; `404` em registro inexistente (a não idempotência do status fica declarada na documentação); `422` com `validation_error` em recusa do BC, inclusive por integridade referencial, sem classificar erro por texto de mensagem. Quatro camadas anti acidente: opt-in com padrão desligado; confirmação consciente ao marcar, reaproveitando `RequiresGenerationConfirmation`, com aviso de que apagar o cabeçalho apaga todas as linhas filhas na mesma transação atômica; `SecurityLevel` próprio por serviço, com aviso destacado quando `None`; e recusa do BC respeitada, sem exclusão forçada. Documentação pública completa após o gate, sem rótulo experimental. **Pontos públicos a revisar quando `B100` fechar**, hoje afirmando o contrário: `README.md`, `README.en.md` e `README.es.md`, na lista *Limitações honestas* ("Sem serviço `DELETE` no MVP", nas três traduções), e `Docs/Public/DEMO.md`, em *Limitações honestas da Alpha*. Sem essa revisão, o release `0.1.0-alpha.6` — o corte próprio do `Delete`, definido na revisão de plano de 2026-08-23 — sairia com a documentação pública negando, em três idiomas, um serviço que passou a existir. **Acrescentado e resolvido em 2026-08-24:** o critério de aceite do documento 27 afirmava de forma absoluta que "não há endpoint `Delete` no MVP", enquanto a seção 6 do mesmo documento e o documento 15 já condicionavam o `Delete` ao `B100` opt-in. O critério foi corrigido na mesma data e hoje lê "não há endpoint `Delete` por padrão; quando o `B100` estiver implementado e o serviço estiver marcado no Wizard, valerão seus critérios próprios de `200`, `404` e `422`". Não integra mais a lista de pontos a revisar quando `B100` fechar; os três `README` e o `DEMO` continuam integrando.
