...
| B115 | Não há caminho de limpeza para API gerada **sem metadata**: `Remover` e `Sincronizar` bloqueiam por metadata ausente e o `Wizard` degrada, e é o próprio pipeline que cria esse estado quando a interrupção cai entre a gravação do API Object e a da metadata | Alta — medido em 2026-09-05 na `Empresa` de `fabricabrasil18test` (`Docs/Implementation/2026-09-04-B111-SONDAS-IDENTIDADE-E-DIARIO.md` §10.6); deixou 49 objetos órfãos, com limpeza apenas manual objeto a objeto na IDE. Relacionado a `B110` (que produz o estado) e a `B111` (cuja F1 encurta a janela mas não a fecha). Exige um caminho de remoção que não dependa da metadata, com posse verificável por outra via — a seção 5.6.2 da F3 trata só de legado herdado e suas duas saídas colapsam em "bloquear" neste caso. **Workaround testado e insuficiente** (§10.7): apagar manualmente só o API Object destrava o Wizard e recupera o contrato, mas a mesma falha de `B109` reaparece na etapa seguinte e recria o estado sem saída |
