...
| B115 | Não há caminho de remoção direta para API gerada **sem metadata** | Alta — o caso foi medido em 2026-09-05 na `Empresa` de `fabricabrasil18test` (`Docs/Implementation/2026-09-04-B111-SONDAS-IDENTIDADE-E-DIARIO.md` §10.6). A opção explícita de recuperação de metadata órfã agora cria somente `apiEmpresa_Metadata`, com confirmação do usuário e posse validada, permitindo depois usar `Remover API gerada`; ela não é remoção direta nem fecha a janela de atomicidade entre Saves. O item permanece aberto para a solução durável de remoção/recuperação independente da metadata, relacionada a `B110` e `B111`. Evidência da opção e do experimento: `Docs/Implementation/2026-09-05-ENCERRAMENTO-BC-EMPRESA.md` |
