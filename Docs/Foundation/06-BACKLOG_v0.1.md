...
| B107 | `Test-OpenApiClientContractValidity.ps1` valida YAML de referência fora do repositório, hoje desatualizado — check do gate mecânico dando falso verde | Alta — antes da Fase 0; planejado |
...
**B107 — YAML de referência do gate mecânico está fora do repositório e desatualizado.** `Tests/OpenApiContract/Test-OpenApiClientContractValidity.ps1` lê dois caminhos absolutos que só existem na máquina do mantenedor: `C:\KBs\wsEducacaoSpTeste\NETFrameworkSQLServer004\web\apiNotaFiscal.yaml` e o equivalente em `NETPostgreSQL155`. Medido em 2026-08-24: os dois arquivos são de **2026-08-16**, anteriores ao `B102`, e a palavra `Messages` não aparece em nenhum deles. O teste passa porque assere a presença de `sdt_API_ErrorResponse:` e dos SDTs próprios, sem exigir o membro coleção — ou seja, valida o contrato de duas semanas atrás e **não** o que o gerador produz hoje. É um check do gate dando falso verde, e o problema não é do `Delete` nem do `B102`: qualquer mudança futura de contrato passa despercebida por ele. Três eixos a decidir na frente: o artefato de referência precisa entrar no repositório ou o teste precisa deixar de depender de caminho de máquina; a asserção precisa acompanhar o contrato vigente; e vale avaliar se este teste se funde com a linha de base da Fase 0, que resolve o mesmo problema por arquivos de referência versionados. Encontrado na revisão pré-push retroativa de 2026-08-24.
...
**B100 — serviço `Delete`.** Frente própria, executada após a Fase 7. Contrato: `200` com a chave primária removida; `404` em registro inexistente (a não idempotência do status fica declarada na documentação); `422` com `validation_error` em recusa do BC, inclusive por integridade referencial, sem classificar erro por texto de mensagem. Quatro camadas anti acidente: opt-in com padrão desligado; confirmação consciente ao marcar, reaproveitando `RequiresGenerationConfirmation`, com aviso de que apagar o cabeçalho apaga todas as linhas filhas na mesma transação atômica; `SecurityLevel` próprio por serviço, com aviso destacado quando `None`; e recusa do BC respeitada, sem exclusão forçada. Documentação pública completa após o gate, sem rótulo experimental. **Pontos públicos a revisar quando `B100` fechar**, hoje afirmando o contrário: `README.md`, `README.en.md` e `README.es.md`, na lista *Limitações honestas* ("Sem serviço `DELETE` no MVP", nas três traduções), e `Docs/Public/DEMO.md`, em *Limitações honestas da Alpha*. Sem essa revisão, o release `0.1.0-alpha.6` — o corte próprio do `Delete`, definido na revisão de plano de 2026-08-23 — sairia com a documentação pública negando, em três idiomas, um serviço que passou a existir. **Nota de 2026-08-24 — o condicionamento antecipado foi revertido.** Entre 2026-08-23 e 2026-08-24, o documento 15 e o critério de aceite do documento 27 passaram a condicionar o `Delete` ao serviço "estar marcado no Wizard", descrevendo como vigente uma UI que não existe: a lista de serviços gerados é fechada em `List`, `Get`, `Create` e `Update` (`PrototypeWizardContract.ServiceNames`), sem checkbox, Procedure ou rota de `Delete`. Um critério que já prevê o caso não reprova uma implementação parcial — trocava-se uma trava por um plano, antes de a frente existir. Ambos foram restaurados à forma absoluta, com o condicionamento marcado explicitamente como estado futuro. Quando o `B100` fechar, os dois documentos entram na revisão junto com os três `README` e o `DEMO`.
