...
O texto gerado é `{apiName} - by Genexus Open API Builder` (sem ponto terminal, padrão usual de Description de objeto GeneXus). A forma legada com ponto final e a prosa antiga `REST API for <Transaction>...` permanecem aceitas no fallback B087 e na integridade B067.

A mesma string continua sendo a sentinela de posse comparada por igualdade exata antes de qualquer reescrita (quando não há metadata), e é registrada na integridade B067. Essa acumulação de papéis é dívida conhecida e ficou mais arriscada depois que o texto passou a ser legível. `B087` separa os dois papéis (posse na metadata; `Description` editável), foi implementado e validado no U15 em 2026-08-07; ver `Docs/Implementation/B087-POSSE-API-OBJECT-METADATA.md` e a nota operacional no documento 06. Com B087, a `Description` pode ser alterada manualmente sem perder a posse, desde que o File de metadata exista e esteja íntegro.
