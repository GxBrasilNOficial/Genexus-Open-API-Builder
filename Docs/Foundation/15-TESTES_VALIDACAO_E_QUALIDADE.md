...
**Quanto a subníveis (B095–B099):** o critério abaixo é da frente completa, não do runtime atual. Hoje (`B097`): o plano offline emite SDTs derivados e o Source BC de Get/Create/Update com `<Subnível>Replace` quando `ApiPlan.Levels` tem filhos; **não** emite `ListResponse_Item`; o Wizard não popula `Levels`, então a geração na IDE continua só o primeiro nível. `B098` cobre `ListResponse_Item` e contadores.
