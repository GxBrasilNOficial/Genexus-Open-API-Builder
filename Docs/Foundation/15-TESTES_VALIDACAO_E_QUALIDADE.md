...
**Quanto a subníveis (B095–B099):** o critério abaixo é da frente completa, não do runtime atual. Hoje (`B099b` concluído em 2026-08-28): metadata V2 hierárquica, Sync e Remover operacionais; Wizard poda `Levels`; plano offline emite SDTs derivados, Source BC com `<Subnível>Replace` e `List` com `ListResponse_Item` + contadores; smoke HTTP multinível validado na Fase 5-A (`B099v`). Required de linha é só UI. Residual consciente: detector de conflito SDT no Sync ainda flat vs raiz hierárquico (Fase 7). Gates `tests.wizardHierarchical`, `tests.sdtHierarchicalPlan`, `tests.businessComponentHierarchical`, `tests.listHierarchical`, `tests.metadataHierarchical`.
