...
**Quanto a subníveis (B095–B099):** o critério abaixo é da frente completa, não do runtime atual. Hoje (`B096`): o plano offline emite os SDTs derivados Create/Update/Response quando `ApiPlan.Levels` tem filhos; **não** emite `ListResponse_Item`; o Wizard não popula `Levels`, então a geração na IDE continua só o primeiro nível. `B097` cobre rotas/Source com `<Subnível>Replace`; `B098` cobre `ListResponse_Item` e contadores.
