...
**Quanto a subníveis (B095–B099):** o critério abaixo é da frente completa, não do runtime atual. Hoje (`B099a`): o Wizard poda `Levels` e o apply hierárquico é permitido; o plano offline já emite SDTs derivados, Source BC com `<Subnível>Replace` e `List` com `ListResponse_Item` + contadores. Metadata/sync/remoção continuam V1 até `B099b`. Required de linha é só UI. Gate `tests.wizardHierarchical`.
...
**Onde esse critério é exercitado — 2026-08-27.** A parte de **geração** do parágrafo acima (SDTs derivados por contrato e `ListResponse_Item`) já está coberta offline pelos gates `tests.sdtHierarchicalPlan`, `tests.businessComponentHierarchical` e `tests.listHierarchical`. A parte de **rotas** — coleções preenchidas no `Get`, substituição de linhas só com `<Subnível>Replace = True`, preservação quando o marcador está ausente, contadores na listagem e ausência dos membros de coleção em `items` — é o roteiro do item 2 da **Fase 5-A (`B099v`)**: reinstalar a DLL, reaplicar o Wizard na `Teste` de quatro níveis e só então o smoke HTTP nos dois environments — a `apiTeste` hoje na KB precede mudanças de emissor e mediria o gerador antigo. Até esse smoke, `Build All` é a única evidência de runtime que existe para o caminho hierárquico, e ele prova especificação e compilação, não comportamento.
