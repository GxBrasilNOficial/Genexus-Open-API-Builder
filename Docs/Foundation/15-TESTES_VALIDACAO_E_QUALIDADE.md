...
**Quanto a subníveis (B095–B099):** o critério abaixo é da frente completa, não do runtime atual. Hoje (`B098`): o plano offline emite SDTs derivados, Source BC de Get/Create/Update com `<Subnível>Replace` e `List` com `ListResponse_Item` + contadores quando `ApiPlan.Levels` tem filhos; o Wizard não popula `Levels`, então a geração na IDE continua só o primeiro nível. `B099` cobre Wizard/metadata.
