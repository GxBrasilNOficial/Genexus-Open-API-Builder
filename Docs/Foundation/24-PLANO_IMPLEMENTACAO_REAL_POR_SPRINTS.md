...
- `B082`: sinal de vida no Wizard (abertura/apply) e no Remover — **missão de outra sessão**; fora da linha de corte do MVP e fora do corte `0.1.0-alpha.5`; recado `Docs/Implementation/2026-08-29-UX-PROGRESSO-WIZARD-APPLY.md`
...
5. **Serviço `Delete` (`B100`) — gate pendente:** quando a frente for executada, comprovar por HTTP real nos dois environments `200` com a chave removida, `404` em registro inexistente e `422` com `validation_error` em recusa do Business Component, inclusive por integridade referencial. Comprovar também na IDE as quatro camadas anti acidente: opt-in desligado por padrão, confirmação consciente ao marcar, `SecurityLevel` próprio com aviso quando `None`, e recusa do BC respeitada sem exclusão forçada.
