...
- `B082`: sinal de vida no Wizard, Sync e Remover — **Fases A+B em código**; Preview do Sync (2026-09-01); corte `0.1.0-alpha.7` **publicado em 2026-09-01**; registro `Docs/Implementation/2026-08-31-B082-PLANO-UX-PROGRESSO.md`. Etapa 1A **aceita** em 2026-09-03 (`Docs/Implementation/2026-09-03-B082-ETAPA-1A-ACEITE.md`). Após a retomada de 2026-09-16, Etapas **2** e **3** fechadas; **Remissão — 2026-09-17:** Etapa 1B aceita com ressalva de relógio e residual `B082` **fechado** (`Docs/Implementation/2026-09-17-B082-ETAPA-1B-ACEITE.md`); a F1 da sprint `S-B111` foi autorizada, implementada e recebeu validação manual para encerramento com a exceção explícita do `B121` em 2026-09-10/11 (`Docs/Implementation/2026-09-10-S-B111-F1-RECONCILIACAO-EVIDENCIA.md`). A F2 (seam de persistência e recibos) foi encerrada em 2026-09-13, após testes offline e na IDE. A F3 (durabilidade e remoção) foi **encerrada em 2026-09-15**, com a sprint: P0 (metadata V3) e P1 (schema V1 do diário) em 2026-09-14, offline; P2 (o diário gravado na KB em Apply e Sync) validada na IDE na mesma data, em duas KBs; e P3 (gate estendido e precedência de `GateDiagnostic`) validada na IDE em 2026-09-14, em seis cenários — os cinco do roteiro e um sexto nascido da própria bateria —, com quatro correções saídas da validação — entre elas a fronteira `ApiPhysicallySaved`, que era registrada sem gravação do API Object, e a identidade composta, que apontava para o próprio objeto em vez da API. As etapas P4 a P7 — remoção com intenção e fila por passadas, serviços de recuperação, comando explícito com preferência, e localização trilíngue com gates novos — foram implementadas **offline** na mesma data; a P8 começou em seguida e **os nove cenários foram exercidos até 2026-09-15 — oito passaram e um foi reformulado pelo que mediu**, porque o envelope apenas preparado não é alcançável pela interface —, produzindo correções de diagnóstico e apresentação, uma delas num defeito publicado na `0.1.0-alpha.7` (`Docs/Implementation/2026-09-14-S-B111-F3-P8-VALIDACAO-IDE.md`). A continuação de `Apply` e `Sync` interrompidos ficou declaradamente fora do entregue, porque o envelope não carrega o contrato — esses envelopes passaram a poder ter o **registro encerrado** por decisão humana informada, com estágio terminal próprio e sem apagar objetos —, e remover duas vezes a mesma API passou a bloquear em vez de ser idempotente (`Docs/Implementation/2026-09-14-S-B111-F3-P4-P7-IMPLEMENTACAO-OFFLINE.md`). Condição de publicação registrada no documento 06: um corte com a P2 dentro exige a validação da P8 ou a documentação pública do diário. `B108` está estacionado.
