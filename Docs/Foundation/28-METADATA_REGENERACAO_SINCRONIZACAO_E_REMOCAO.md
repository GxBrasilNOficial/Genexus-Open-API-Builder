...
**Inventário próprio deixa de ser fixo (Fase 7).** "SDTs próprios" passa a incluir os derivados de subnível (`sdt<NomeBase>_API_<Papel>_<Subnível>`) e, quando houver subnível selecionado, o `sdt<NomeBase>_API_ListResponse_Item`. A remoção lê esses nomes da metadata em vez de assumir lista fechada, sob pena de deixar órfãos na KB. A ordem de exclusão continua respeitando a dependência entre tipos. **Hoje (`B086`, inalterado em `B096`):** o plano de remoção ainda monta `OwnSdtNames` só com os cinco campos fixos da metadata (`listResponse`, `createRequest`, `updateRequest`, `listFilters`, `response`).
