...
- Correção 2026-08-17: o API Object não é mais procurado somente como `api<Transaction>`. Com metadata própria, `api.name` (ou `ownership.apiName`) seleciona o objeto customizado cujo Service Source deve ser lido. Sem metadata, a descoberta exige Description própria e chamada a `proc<Transaction>_API_<Serviço>`; o nome convencional e os candidatos customizados são avaliados juntos, e ambiguidade não escolhe objeto arbitrariamente. Cobertura mecânica nos testes de contrato/parser; validado no U15 com `ApiName=apiFiscalPublica` e metadata `apiNotaFiscal_Metadata`;
