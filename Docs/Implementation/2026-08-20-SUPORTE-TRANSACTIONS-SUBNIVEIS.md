...
- A leitura via GeneXus SDK navega recursivamente pela árvore de TransactionLevels a partir de 	ransaction.Structure.Root.
...
### 6. Contrato de Erros para Subníveis (Alinhamento com a Emenda 2026-08-03)
- O SDT compartilhado sdt_API_ErrorResponse permanece top-level (Code e Message), sem array de erros aninhados.
- Se qualquer regra do BC falhar em uma linha filha durante o Save():
  - O status HTTP retornado é 422 Unprocessable Content (ou 400 Bad Request);
  - ErrorResponse.Code retorna "validation_error";
  - ErrorResponse.Message retorna a mensagem descritiva emitida pelo Business Component (&BC.GetMessages().Item(1).Description);
  - Não é emitido array paralelo por linha, mantendo conformidade estrita com o contrato OpenAPI publicado.
...
- O comparador de sincronização (Sincronizar com a Transaction) compara adições, renomeações e remoções dentro da hierarquia de níveis com base nos ttributeGuids.
...
5. **Validação HTTP:** Chamadas reais POST, GET, PUT e GET (List) validando persistência, substituição de linhas, contadores e mensagens de erro do BC.
