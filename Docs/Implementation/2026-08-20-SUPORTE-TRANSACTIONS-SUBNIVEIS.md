...
9. **Contrato OpenAPI publicado, ao fim da Fase 4** e não na véspera do corte, para que ainda haja sprint para reagir:
   ...
   **Prazo vencido — realocado em 2026-08-27.** A Fase 4 fechou em 2026-08-26 e a Fase 5 fechou no mesmo dia; das três partes abaixo, só a trava mecânica offline foi executada. A conferência manual do YAML e a geração de cliente não foram feitas nem registradas como pendência, e o item existia justamente para não cair na véspera do corte. Passam a ser executadas na **Fase 5-A (`B099v`)**, sobre a `apiTeste` de quatro níveis **regerada** naquela fase — a versão hoje na KB é de 2026-08-26 e precede as mudanças de gerador de `8f80f39`. O critério permanece escrito aqui como está, para preservar o que foi decidido em 2026-08-23.
