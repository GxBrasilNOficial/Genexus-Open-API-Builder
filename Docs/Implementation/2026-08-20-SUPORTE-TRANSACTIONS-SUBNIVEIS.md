...
9. **Contrato OpenAPI publicado, ao fim da Fase 4** e não na véspera do corte, para que ainda haja sprint para reagir:
   - a trava mecânica offline `Tests/OpenApiContract/Test-OpenApiClientContractValidity.ps1` (somente `ApiPlan.cs` no repositório, desde `B107`) passa a reconhecer também os padrões de nome derivados (`_API_Response_<Subnível>`, `_API_CreateRequest_<Subnível>`, `_API_UpdateRequest_<Subnível>`, `_API_ListResponse_Item`) quando a geração multinível existir no plano. Sem isso ela seguiria passando enquanto o contrato muda por baixo;
...
**Ordem de execução resultante:** `B102` (concluído) → Fase 0 (concluída nas camadas de início; conferência de fim permanece) → Fase 1/`B095` (concluída) → Fases 2 a 6 → Fase 7 → `B100`.
