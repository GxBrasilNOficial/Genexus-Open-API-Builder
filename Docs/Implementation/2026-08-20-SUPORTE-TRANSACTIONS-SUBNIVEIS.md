...
9. **Contrato OpenAPI publicado, ao fim da Fase 4** e não na véspera do corte, para que ainda haja sprint para reagir:
   - a trava mecânica offline `Tests/OpenApiContract/Test-OpenApiClientContractValidity.ps1` lê `ApiPlan.cs` (padrões de cabeçalho e serviços, desde `B107`) e, desde `B096`, também `ApiPlanSdtHierarchicalNaming.cs` para `_API_CreateRequest_`, `_API_UpdateRequest_` e `_API_Response_`. `_API_ListResponse_Item` ainda não está no naming nem no plano; entra nessa trava quando `B098` existir no código. Sem esses padrões ela seguiria passando enquanto o contrato muda por baixo;
