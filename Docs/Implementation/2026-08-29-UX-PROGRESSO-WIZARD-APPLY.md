...
3. Sinal na abertura do Wizard (casca + tempos `PrefsMs` / `ContratoMs` / `InterfaceMs` na Output). O índice único da KB **não** entra nessa abertura: na `Empresa` (2026-08-31) `IndiceKb=2394` ms foi no Apply, depois de Concluir. Preview do Sync e do Remover usam casca + índice **antes** da confirmação.
