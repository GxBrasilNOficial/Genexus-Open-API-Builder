...
em 2026-09-05**: `B108` foi estacionado e a próxima ação única passou a ser decidir o encerramento
 ...
 recebeu validação manual para encerramento com a exceção explícita do `B121` em 2026-09-10/11. O residual 1B/2/3 permanece neste plano. Após o encerramento da `S-B111`,
o checkpoint deverá reavaliar esse residual; a `S-B111` não fecha automaticamente o `B082`.
...
1. **Escrita parcial do BC (P1 — consistência B067).** Em `ApiPlanBusinessComponentWriter.Apply`, `saveSteps` grava o API Object **antes** das Procedures (`SaveApi` → `SaveProcedure` Get/Create/Update/Delete). Se o Sync ou o Apply com BC aborta no meio, o Service Source do `api<Nome>` fica à frente do hash em `api<Nome>_Metadata`; o preflight seguinte bloqueia Wizard e Sync com `BaselineServiceSourceHashMismatch` até **Remover API gerada + Wizard**. Evidência: Sync Keep interrompido em `wsEducacaoSpTeste` / `NotaFiscal` (2026-09-03). **Correção recomendada:** reordenar `saveSteps` para gravar o API Object **por último**, após todas as Procedures passarem em `Save()`. Normativo: `Docs/Implementation/B085-SINCRONIZAR-COM-TRANSACTION.md` (seção «Escrita parcial do BC»). **Destino da frente:** este P1 saiu do residual operacional do `B082` e passou a compor a sprint **`S-B111`**, em F1/F2/F3. A F1 foi autorizada e implementada; recebeu validação manual para encerramento com a exceção explícita do `B121` em 2026-09-10/11. A matriz de Sync permanece com a exceção do perfil List isolado, registrada no `B121`; a F2/F3 permanecem planejadas. Não tratá-lo como Etapa 2 deste plano nem como tarefa posterior a `B108`; após o encerramento da `S-B111`, reavaliar o residual 1B/2/3 do `B082` no checkpoint. Ver a reconciliação em `Docs/Implementation/2026-09-10-S-B111-F1-RECONCILIACAO-EVIDENCIA.md`.
...
3. **Higiene de teste (não bloqueia).** `Tests/SdtReencounter/Test-SdtCollectionItemNameProbe.ps1` não está no orquestrador pré-push e sobrepõe asserções de `Tests/KbIndexReuse/Test-ApiPlanKbIndexReuse.ps1` (gate `tests.kbIndexReuse`). **Ação:** absorver as asserções únicas do probe (`CollectionItemName` vazio, `NormalizeSdtTypeName`) no KbIndexReuse; remover `Tests/SdtReencounter/`; **não** criar gate novo. Ainda nesta sprint, no próximo toque em testes/pré-push.
