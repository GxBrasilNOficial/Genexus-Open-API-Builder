...
B050-B053 estão concluídos como comando separado e como etapa integrada ao wizard. B054 foi validado posteriormente: `apiCarga` expõe os serviços que delegam para estas Procedures. B055 também foi validado posteriormente: Create e Update deixaram de permanecer como skeleton quando a opção Business Component é aplicada e passaram a usar a Transaction via BC nas Procedures próprias. B056 também foi validado posteriormente: as descrições resolvidas no `ApiPlan` são aplicadas no API Object real. A próxima frente canônica deve ser consultada em `Docs/STATUS_ATUAL_E_PROXIMO_PASSO.md`.
