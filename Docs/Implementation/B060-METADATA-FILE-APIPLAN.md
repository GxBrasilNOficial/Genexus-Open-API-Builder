...
- A relitura do JSON para conferir esse fingerprint usa `DateParseHandling.None`. `JObject.Parse` (padrao Newtonsoft) converte `generatedAtUtc` ISO-8601 em `DateTime` e, ao compactar de novo, corta zeros fracionarios de `ToString("O")`, o que falsificava o SHA-256 sem adulterar o File.
