...
Na revisao seguinte, foi identificado o inverso do mesmo risco de navegacao: uma habilitacao explicita marcada na aba `Business Component` ainda podia deixar de executar se o usuario clicasse diretamente em `Resumo`, sem usar `Proximo` nessa aba. O wizard passou a centralizar a execucao de acoes explicitas pendentes antes de montar/exibir o resumo e antes de concluir, preservando a liberdade de clicar diretamente nas abas sem perder uma decisao confirmada pelo usuario.

O teste manual seguinte confirmou que a habilitacao explicita passou a executar antes de exibir o `Resumo`, mas tambem revelou que a intencao de aplicar Create/Update via Business Component podia ser perdida: enquanto BC ainda estava desabilitado, o controle de B055 era desmarcado pelo estado visual bloqueado e nao era restaurado depois da habilitacao. O wizard passou a manter uma intencao pendente de `ApplyBusinessComponent`; quando a Transaction fica apta via BC, essa intencao reativa a confirmacao de B055 para a mesma conclusao do wizard.

Na revisao visual da aba, foi identificado que o usuario tambem precisava conseguir marcar manualmente a aplicacao B055 antes da gravacao de BC. A aba passou a liberar `Aplicar Create/Update via Business Component apos habilitar` quando `Habilitar Business Component agora` estiver marcado. Isso registra apenas uma intencao pendente; a alteracao da Transaction continua condicionada ao modal exibido ao ir para `Resumo` ou concluir.

O teste com preferencias ausentes confirmou que o wizard deve continuar abrindo com defaults conservadores, mas tambem mostrou que B055 nao deve ficar concluivel isoladamente quando SDTs, Procedures e API Object nao estao confirmados nem reencontrados. A UI foi alinhada ao preflight tecnico: `Aplicar Create/Update via Business Component` so fica habilitado quando BC esta apto ou pendente de habilitacao explicita e as dependencias SDTs, Procedures e API Object estao disponiveis ou selecionadas no mesmo wizard.

Depois desse ajuste, a ordem das abas tambem foi alinhada ao fluxo de dependencia: `Business Component` passou a ficar depois de `SDTs`, `Procedures` e `API Object`, antes de `List`, `Metadata` e `Resumo`. A mudanca e apenas de navegacao; o contrato de preflight e as flags de geracao permanecem os mesmos.

Na revisao pre-push, foi identificado um caminho de UI ainda fragil: `Resumo` podia acionar a habilitacao explicita de BC, mas o refresh local da aba `Business Component` ocorria antes de `TryCreateSelection()` e sem conhecer as dependencias ja marcadas. O wizard passou a recalcular o preview completo depois de uma habilitacao bem-sucedida e antes de montar a selecao final, preservando `ApplyBusinessComponent=True` quando SDTs, Procedures, API Object e B055 estavam confirmados.

A cobertura automatizada foi ampliada sem acoplar o teste a WinForms: `PrototypeWizardBusinessComponentNavigationPolicy` agora cobre a sequencia em que um refresh local sem dependencias limpa o checkbox visual, mas um refresh completo com SDTs, Procedures e API Object disponiveis restaura a intencao pendente de B055 antes de `TryCreateSelection()`.

O teste manual curto recomendado para esse caminho ja havia sido executado na IDE com `Distribuidora`: a Transaction iniciou sem Business Component, o wizard teve SDTs, Procedures, API Object e B055 confirmados, o usuario clicou diretamente em `Resumo`, confirmou a habilitacao de BC, o resumo exibiu `Aplicar Create/Update via Business Component: True`, a Output registrou `B034 ... ApplyBusinessComponent=True` e B055 foi aplicado com sucesso.
...
- `PrototypeWizardBusinessComponentNavigationPolicy`: passagem pela aba `Business Component` ou abertura direta do `Resumo` nao deve bloquear quando a Transaction nao e BC e a habilitacao explicita nao foi marcada; deve pedir confirmacao antes de resumo/conclusao quando a habilitacao explicita foi solicitada; deve permitir selecionar B055 como intencao pendente quando a habilitacao explicita foi marcada e SDTs, Procedures e API Object estao disponiveis ou confirmados; deve preservar a intencao de aplicar B055 quando a Transaction fica apta via BC durante o proprio wizard;
