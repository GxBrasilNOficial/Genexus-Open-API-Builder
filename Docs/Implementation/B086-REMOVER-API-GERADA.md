...
5. Apaga nesta ordem: API Object → Procedures → SDTs próprios (ListResponse antes de Response) → Metadata File → Folder (só se `ownedByThisApi=true`, GUID persistido coincidir quando existir, Description canônica ou legada, contêiner esperado e ficar vazio). A IDE bloqueia exclusão quando ainda há referência; por isso o dependente sai antes do referenciado. **Desde 2026-09-06 a ordem dos SDTs próprios é uma preferência, não um requisito:** o que a IDE recusar volta para a fila e é tentado na passada seguinte; enquanto cada passada apagar ao menos um objeto há progresso, e uma passada inteira sem progresso encerra a remoção listando os pendentes com o motivo de cada um. A recusa não é interpretada pela mensagem da exceção — só o aborto do usuário é relançado na hora. Isso existe porque a metadata reconstruída pela recuperação de órfã (`B115`) não tem como reproduzir a ordem original, e uma lista fora de ordem interrompia a remoção no meio, com objetos já apagados. As checagens de posse/ambiguidade se repetem em cada etapa como defesa; se o estado da KB mudar após o preflight, a remoção interrompe sem novas exclusões, e o relatório final passa a listar o que já havia saído da KB (residual de falha IDE/SDK no meio da sequência permanece).
...
7. Folder: sem posse histórica → “reutilizado; nunca apagar”; criado nesta execução → “criado pela extensão; apagar só se ficar vazio”; reencontrado com posse (`ownedByThisApi=true`, `wasCreated=false`) → “próprio da API; a remoção apaga se ficar vazio”. Description editada (não canônica/legada) ou contêiner inesperado **preservam** o Folder e a confirmação anuncia «será preservado — Description ou contêiner divergente» (`B126`); o relatório B081 emite aviso tipado se a execução preservar por esse motivo.

Preserva: Transaction, Business Component, Folder sem posse histórica (e Folder próprio que falhe Description/contêiner/vazio), SDTs `sdt_API_*` compartilhados.
...
A regra `wasCreated` **deixou de ser a autorização** para apagar o Folder. O flag continua no JSON e no relatório desta execução; quem enfileira é `ownedByThisApi`, com conferência de `guid` quando o campo existe. Metadata V1–V3 com `wasCreated=true` ainda enfileira até a primeira regravação V4. B115 continua sem reivindicar dono. Plano: `Docs/Implementation/2026-09-20-B123-PLANO-POSSE-HISTORICA-FOLDER.md`.
