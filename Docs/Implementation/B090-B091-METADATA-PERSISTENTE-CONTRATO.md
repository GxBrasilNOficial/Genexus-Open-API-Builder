# B090/B091 - Contrato minimo da metadata persistente futura

Validado manualmente no GeneXus 18 Upgrade 15 em 2026-07-25: o `ApiPlan` passa a preservar o contrato minimo da futura metadata persistente de configuracao por KB para sensiveis e auditoria, ainda sem ler ou gravar File de metadata.

## Objetivo

Definir quais dados B090/B091 deverao ser carregados de metadata persistente para substituir `DefaultInMemoryHardcodedB090B091Policy` em frente futura, mantendo a classificacao atual somente em memoria.

## Escopo implementado

- `PrototypeWizardFieldClassificationMetadataContract` define o contrato futuro no snapshot do wizard;
- `ApiPlanFieldClassificationMetadataContract` preserva esse contrato no `ApiPlan`;
- `SchemaVersion` fica `B090B091_KB_FIELD_CLASSIFICATION_V1`;
- a secao futura do JSON fica `fieldClassification`;
- o membro de sensiveis exatos fica `sensitiveExactNames`;
- o membro de auditoria por nome exato fica `auditExactNames`;
- o membro de auditoria por sufixo fica `auditSuffixes`;
- os membros obrigatorios V1 ficam `schemaVersion`, `fieldClassification`, `sensitiveExactNames`, `auditExactNames` e `auditSuffixes`;
- a Output B090/B091 passa a exibir esse contrato da metadata futura;
...
- o `ApiPlan` preserva schema, secao e membros minimos da metadata futura;
- a classificacao atual continua em memoria e ainda usa `DefaultInMemoryHardcodedB090B091Policy`;
- a Output declara que nenhum File de metadata foi lido ou gravado;
- B090/B091 canonicos continuam abertos ate carregarem regras explicitas por KB a partir de metadata persistente real.
