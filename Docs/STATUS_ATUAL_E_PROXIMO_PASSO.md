...
**Sprint 5 — Procedures, API Object e Metadata**, com B040-B046, B050-B053, B054, B055, B056 e B060 validados no encerramento integrado do wizard. A próxima frente operacional é validar B061/B062 sobre a organização no módulo da Transaction e a nomenclatura padrão dos objetos já gerados, antes de aprofundar B063/B064.

## Próxima ação única

Validar B061/B062 em KB de teste: confirmar que os objetos específicos da API gerada ficam no mesmo módulo da Transaction, no Folder específico esperado quando aplicável, e que os nomes persistidos seguem as convenções oficiais congeladas.

## Critério de conclusão e evidência esperada

- os objetos específicos `apiContrato`, `procContrato_API_*`, `sdtContrato_API_*` e `apiContrato_Metadata` permanecem associados ao módulo da Transaction `Contrato` e ao Folder específico `ContratoOpenApi` quando o SDK público permitir;
- os SDTs compartilhados `sdt_API_ErrorResponse` e `sdt_API_Pagination` permanecem no escopo compartilhado esperado em `GxOpenAPI`;
- os nomes efetivos conferem com as convenções B062: `api<NomeBase>`, `api<NomeBase>_Metadata`, `proc<NomeBase>_API_<Servico>` e `sdt<NomeBase>_API_<Finalidade>`;
- a Output da IDE e/ou inspeção visual confirma que a validação é de organização/naming, sem completar REST, códigos HTTP finais, segurança definitiva ou ciclo completo de regeneração.
