...
- B060 validado manualmente em recorte inicial em 2026-07-27 na Transaction `GuiaPed`: o wizard exibiu a aba `Metadata B060`, manteve a confirmação desmarcada por segurança, gravou/reencontrou o File `apiGuiaPed_Metadata`, corrigiu a persistência do `External File Name` via `BlobPart.FileName`, abriu o File na IDE sem erro e exportou `D:\Temp\apiGuiaPed_Metadata.json`. O JSON exportado parseou com `schemaVersion='GOAB_API_METADATA_B060_V1'`, Transaction `GuiaPed`, API `apiGuiaPed`, 4 serviços, 4 Procedures, 2 SDTs compartilhados, 21 campos de Create, 21 de Update, 53 de Response, 2 filtros List, 42 entradas de obrigatoriedade, descrições B056 e escopo limitado sem REST completo, códigos HTTP finais ou segurança definitiva. O preflight visual bloqueou upload de substituto com JSON inválido e upload com `ownership.apiName` incompatível, mantendo a confirmação desabilitada e sem escrita; após restaurar o JSON válido, voltou para `Reencontrar e validar`; a reexecução final, feita logo depois da restauração, reencontrou o File com `Guid='1b943d80-5961-4b25-a89a-2454dae9f45d'`, `Bytes=99432` e `Sha256='C518A9EEFF4F9785E5B63D27F92D1E451D2E70B8E0FCE5BE8F98472D6095DF54'`; o `Build All` executado em seguida passou, sem objetos para especificar, mantendo apenas o warning ambiental de `FBiTextSharp.dll`. Hash SHA-256 do arquivo exportado: `D2F16C9CCB66694911AE4EB31F8399627AC2562F40C067CC7828B869C809081E`.

## Frente ativa

**Sprint 5 — Procedures, API Object e Metadata**, com B040-B046, B050-B053, B054, B055 e B056 validados no encerramento integrado do wizard. B060 tem criação/reencontro, abertura/exportação e bloqueio de JSON inválido ou incompatível validados manualmente em GeneXus 18 U15, mas ainda requer o teste de colisão externa e o caso real de descrições especiais antes de ser encerrado.

## Próxima ação única

Validar os testes negativos restantes de B060 em KB de teste: bloqueio de colisão externa e caso real de descrições especiais B056 com aspas, barra invertida e caracteres incomuns.
...
12. Sprint 5 validou o recorte inicial de B060: metadata persistente em File com JSON versionado, abertura/exportação na IDE, reencontro após restauração do JSON válido e bloqueio conservador de JSON inválido ou incompatível; permanecem pendentes o teste de colisão externa e o caso real de descrições especiais antes de encerrar B060.
13. Sprints 5–7 completam metadata persistente, serviços REST/segurança e o ciclo conservador de conflitos, regeneração e remoção.
14. O marco **wizard funcional do MVP concluído** ocorre ao final da Sprint 7, antes da Alpha.
