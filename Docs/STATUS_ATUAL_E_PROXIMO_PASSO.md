...
- B061/B062 validados manualmente em 2026-07-27 na Transaction `Contrato`: a Output confirmou 5 SDTs próprios em `TransactionModuleFolder:ContratoOpenApi`, 2 SDTs compartilhados em `RootModuleFolder:GxOpenAPI`, 4 Procedures e o API Object `apiContrato` reencontrados em `TransactionFolder='ContratoOpenApi'`; a inspeção visual confirmou `apiContrato`, `procContrato_API_*` e `sdtContrato_API_*` dentro de `ContratoOpenApi`; as Properties do File `apiContrato_Metadata` confirmaram `Module='Root Module'` e `Qualified Name='apiContrato_Metadata'`; a referência `object-file.md` da skill nexa e a wiki oficial confirmaram que File é organizado por módulo, não por Folder; o JSON exportado em `Temp/apiContrato_Metadata.json` parseou com `ownership.transactionModule='Root Module'`, `objects.transactionFolder.name='ContratoOpenApi'`, 4 Procedures, 5 SDTs próprios e 2 SDTs compartilhados com nomes conforme B062.
...
13. Sprint 5 validou B061/B062: API Object, Procedures e SDTs específicos permanecem em `ContratoOpenApi`, SDTs compartilhados permanecem em `GxOpenAPI`, File de metadata permanece no módulo da Transaction por limitação/contrato do objeto File, e todos os nomes persistidos seguem as convenções congeladas.
