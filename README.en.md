...
Public alpha: **[`0.1.0-alpha.9`](https://github.com/GxBrasilNOficial/Genexus-Open-API-Builder/releases/tag/v0.1.0-alpha.9)** — choose the DLL matching your GeneXus version in the Release. **Attention:** the `List` HTTP contract changed (breaking) — see below.
...
- IDE lifecycle: Wizard, Sync with the Transaction, Remove generated API, Recover interrupted operation

### List HTTP contract (since `0.1.0-alpha.9`) — breaking

The `List` body no longer uses the `ListResponse` envelope. Fields are returned at the **root** of the JSON (`Items`, `Pagination`, `AppliedFilters`, `ErrorResponse`). This applies to **every** API regenerated with this version, **whatever generator** the environment uses (including Java). Motivation: the .NET REST wrapper unwraps the SDT when it has a collection; we unified the flat contract in the extension. HTTP smoke in this Alpha on both .NET generators; on Java and others, verify a `GET` after reapplying the Wizard. Callers that read `body.ListResponse…` must update the client. Detail: [0.1.0-alpha.9 notes](Docs/Releases/0.1.0-alpha.9.en.md).

### HTTP error contract (since `0.1.0-alpha.4`)

When the Business Component rejects a rule, `Create` and `Update` respond with **HTTP 422**, `ErrorResponse.Code = validation_error`, the rule text in `Message`, and the `Messages[]` collection (error messages only). For payload validation, any violation responds with **HTTP 400**, all violations are preserved in `Messages[]`, and the top-level `Message` gathers their text. Opt-in `Delete`, when enabled, responds with **HTTP 422** on referential-integrity refusal, using the same SDT. Each API's Source changes only when you reopen the Wizard on it; the shared `sdt_API_ErrorResponse` SDT is unique per KB, so regenerating any API updates the error schema published by all of them. Callers that compared the fixed string `"Business rules rejected the request."` should switch to `Code`. Business-rule details and the opt-out: [0.1.0-alpha.4 notes](Docs/Releases/0.1.0-alpha.4.en.md). Complete payload-validation contract: [document 27](Docs/Foundation/27-CONTRATO_HTTP_ERROS_E_SDTS_COMPARTILHADOS.md). Sublevels and the `<Sublevel>Replace` marker: [0.1.0-alpha.5 notes](Docs/Releases/0.1.0-alpha.5.en.md). Opt-in `Delete`: [0.1.0-alpha.6 notes](Docs/Releases/0.1.0-alpha.6.en.md). Progress B082: [0.1.0-alpha.7 notes](Docs/Releases/0.1.0-alpha.7.en.md). Operation journal and recovery: [0.1.0-alpha.8 notes](Docs/Releases/0.1.0-alpha.8.en.md). Flat List contract: [0.1.0-alpha.9 notes](Docs/Releases/0.1.0-alpha.9.en.md).
...
| Public alpha `0.1.0-alpha.9` | Release package (flat List contract + Remove/Folder hardenings) with U14+ and U13− assets |
| Upgrade 13 and earlier (**U13−**) | Satellite DLL `GenexusOpenApiBuilder.Extension-gx18u13.dll`; formal validation on U13 |
...
Release `0.1.0-alpha.9` contains two DLLs. Install only the one corresponding to your installation:

| File in the GitHub Release | Intended for | Note |
|---|---|---|
| `GenexusOpenApiBuilder.Extension.dll` | GeneXus 18 Upgrade 14, Upgrade 15, and later (**U14+**) | Canonical line; do not use on **U13−** |
| `GenexusOpenApiBuilder.Extension-gx18u13.dll` | GeneXus 18 Upgrade 13 and earlier (**U13−**) | Satellite line; formal validation on U13; do not use on **U14+** |
...
- Interrupted operation: the KB gets the `GxOpenApiBuilder_OperationJournal` File; a non-terminal envelope **blocks** Apply, Sync, and Remove until **Recover interrupted operation** (resume removal, close the record, or discard it). Continuing an interrupted Apply/Sync **does not** exist in this Alpha

## Get started in minutes

1. [Install the extension](Docs/Public/INSTALL.md)
2. [Follow the quick demo](Docs/Public/DEMO.md)
3. Read the [Alpha notes](Docs/Releases/0.1.0-alpha.9.en.md)
...
| [0.1.0-alpha.9](Docs/Releases/0.1.0-alpha.9.en.md) | EN notes; [PT-BR](Docs/Releases/0.1.0-alpha.9.md); [ES](Docs/Releases/0.1.0-alpha.9.es.md) — DLL selection; **breaking List** |
