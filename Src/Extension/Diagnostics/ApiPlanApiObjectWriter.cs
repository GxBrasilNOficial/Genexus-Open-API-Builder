...
{
    private const string OwnedDescriptionGeneratorName = "Genexus Open API Builder";
    ...
    internal static string CreateOwnedDescription(ApiPlan apiPlan)
    {
        ...
        // Sem ponto terminal: Description de objeto GeneXus costuma ser frase sem pontuação final.
        return $"REST API for {apiPlan.TransactionName}, generated by {OwnedDescriptionGeneratorName}";
    }

    /// <summary>
    /// Formas aceitas no fallback B087 e na integridade: atual (sem ponto) e legada (com ponto).
    /// </summary>
    internal static IReadOnlyList<string> CreateOwnedDescriptionCandidates(ApiPlan apiPlan)
    {
        var current = CreateOwnedDescription(apiPlan);
        var legacyWithPeriod = current + ".";
        return new[] { current, legacyWithPeriod };
    }
    ...
    private static (bool OwnedFilePresent, bool Ambiguous, WikiFileKBObject? File) TryFindOwnedMetadataFile(KBModel designModel, ApiPlan apiPlan)
    {
        ...
        var file = matches[0];
        var expectedFileDescription = ApiPlanMetadataFileWriter.CreateOwnedDescription(apiPlan);
        if (!string.Equals(file.Description, expectedFileDescription, StringComparison.Ordinal))
        {
            return (false, false, null);
        }
