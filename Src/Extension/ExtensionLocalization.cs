...
internal sealed class ExtensionTexts
{
    ...
    /// <summary>
    /// Encerrar o registro é a decisão mais delicada da recuperação: ela libera a KB sem
    /// concluir nada. O texto diz isso na cara, e o diagnóstico com o inventário já foi
    /// publicado na Output antes da pergunta.
    /// </summary>
    public string RecoveryConfirmDiscard => Language switch
    {
        ExtensionLanguage.PortugueseBrazil => "A operação registrada gravou objetos na Knowledge Base e parou no meio, e a ferramenta não consegue retomá-la de onde parou. Encerrar este registro libera a KB para as próximas operações e **não apaga nada**: o que ficou pela metade continua exatamente como está, e o inventário permanece gravado no diário. Depois disso, as duas saídas são reaplicar pelo Wizard sobre o estado atual ou remover a API gerada. O que foi encontrado na KB está na janela Output. Encerrar o registro agora?",
        ExtensionLanguage.Spanish => "La operación registrada grabó objetos en la Knowledge Base y se detuvo a la mitad, y la herramienta no puede retomarla desde donde quedó. Cerrar este registro libera la KB para las próximas operaciones y **no borra nada**: lo que quedó a medias sigue exactamente como está, y el inventario permanece grabado en el diario. Después de eso, las dos salidas son reaplicar por el Wizard sobre el estado actual o eliminar la API generada. Lo que se encontró en la KB está en la ventana Output. ¿Cerrar el registro ahora?",
        _ => "The recorded operation wrote objects to the Knowledge Base and stopped halfway, and the tool cannot resume it from where it stopped. Closing this record frees the KB for the next operations and **deletes nothing**: whatever was left halfway stays exactly as it is, and the inventory remains recorded in the journal. After that, the two ways out are reapplying through the Wizard over the current state, or removing the generated API. What was found in the KB is in the Output window. Close the record now?",
    };
    ...
    public string Translate(string source)
    {
        ...
        return Language == ExtensionLanguage.Spanish
            ? source switch
            {
                ...
                "Foi encontrada uma API gerada pela extensao sem o File de metadata '{0}'. A recuperacao criara somente esse File, com o inventario dos objetos encontrados na KB ({1} Procedures, {2} SDTs proprios, {3} compartilhados). Ela devolve a possibilidade de remover a API gerada, mas nao recupera o contrato original: paginacao, ordenacao, campos obrigatorios e a estrutura hierarquica nao existem fora da metadata perdida, e o Sincronizar seguira bloqueado ate uma nova aplicacao completa. Nada mais sera alterado. Deseja recuperar agora?" => "Se encontró una API generada por la extensión sin el archivo de metadatos '{0}'. La recuperación creará solo ese archivo, con el inventario de los objetos encontrados en la KB ({1} Procedures, {2} SDTs propios, {3} compartidos). Devuelve la posibilidad de eliminar la API generada, pero no recupera el contrato original: paginación, ordenación, campos obligatorios y la estructura jerárquica no existen fuera de los metadatos perdidos, y Sincronizar seguirá bloqueado hasta una nueva aplicación completa. Nada más será modificado. ¿Desea recuperar ahora?",
                "A metadata '{0}' foi recuperada. Reabra o Wizard para continuar; nenhuma outra etapa foi executada nesta aplicacao." => "Los metadatos '{0}' fueron recuperados. Abra nuevamente el Wizard para continuar; no se ejecutó ninguna otra etapa en esta aplicación.",
                "A recuperacao da metadata '{0}' falhou: {1}" => "La recuperación de los metadatos '{0}' falló: {1}",
                "O File de metadata '{0}' existe, mas registra um API Object que nao esta mais na KB — tipicamente porque ele foi removido e outro foi gerado com o mesmo nome. Nesse estado o Wizard e o Remover ficam bloqueados. A recuperacao regravara esse File com o inventario atual ({1} Procedures, {2} SDTs proprios, {3} compartilhados) e o API Object que existe agora. O File ja era uma metadata reconstruida, sem o contrato original, entao nada de contrato se perde e o Sincronizar segue bloqueado ate uma nova aplicacao completa. Nenhum API Object, Procedure ou SDT sera alterado. Deseja recuperar agora?" => "El archivo de metadatos '{0}' existe, pero registra un API Object que ya no está en la KB — típicamente porque fue eliminado y se generó otro con el mismo nombre. En ese estado el Wizard y Eliminar quedan bloqueados. La recuperación reescribirá ese archivo con el inventario actual ({1} Procedures, {2} SDTs propios, {3} compartidos) y el API Object que existe ahora. El archivo ya era un metadato reconstruido, sin el contrato original, así que no se pierde ningún contrato y Sincronizar sigue bloqueado hasta una nueva aplicación completa. No se modificará ningún API Object, Procedure ni SDT. ¿Desea recuperar ahora?",
                ...
                "Delete marcado exige Get, Create e Update nos servicos padrao. Marque os tres ou desmarque Delete." => "Delete marcado exige Get, Create y Update en los servicios predeterminados. Marque los tres o desmarque Delete.",
                "Confirmar" => "Confirmar",
            ...
            : source switch
            {
                ...
                "Foi encontrada uma API gerada pela extensao sem o File de metadata '{0}'. A recuperacao criara somente esse File, com o inventario dos objetos encontrados na KB ({1} Procedures, {2} SDTs proprios, {3} compartilhados). Ela devolve a possibilidade de remover a API gerada, mas nao recupera o contrato original: paginacao, ordenacao, campos obrigatorios e a estrutura hierarquica nao existem fora da metadata perdida, e o Sincronizar seguira bloqueado ate uma nova aplicacao completa. Nada mais sera alterado. Deseja recuperar agora?" => "An extension-generated API was found without the metadata File '{0}'. Recovery will create only that File, holding the inventory of the objects found in the KB ({1} Procedures, {2} own SDTs, {3} shared). It restores the ability to remove the generated API, but it does not restore the original contract: pagination, ordering, required fields, and the hierarchical structure exist only in the lost metadata, and Synchronize stays blocked until a full apply. Nothing else will be changed. Recover now?",
                "A metadata '{0}' foi recuperada. Reabra o Wizard para continuar; nenhuma outra etapa foi executada nesta aplicacao." => "Metadata '{0}' was recovered. Reopen the Wizard to continue; no other stage was executed in this application.",
                "A recuperacao da metadata '{0}' falhou: {1}" => "Metadata recovery for '{0}' failed: {1}",
                "O File de metadata '{0}' existe, mas registra um API Object que nao esta mais na KB — tipicamente porque ele foi removido e outro foi gerado com o mesmo nome. Nesse estado o Wizard e o Remover ficam bloqueados. A recuperacao regravara esse File com o inventario atual ({1} Procedures, {2} SDTs proprios, {3} compartilhados) e o API Object que existe agora. O File ja era uma metadata reconstruida, sem o contrato original, entao nada de contrato se perde e o Sincronizar segue bloqueado ate uma nova aplicacao completa. Nenhum API Object, Procedure ou SDT sera alterado. Deseja recuperar agora?" => "The metadata File '{0}' exists, but records an API Object that is no longer in the KB — typically because it was removed and another was generated with the same name. In that state the Wizard and Remove are blocked. Recovery will rewrite that File with the current inventory ({1} Procedures, {2} own SDTs, {3} shared) and the API Object that exists now. The File was already a reconstructed metadata, without the original contract, so no contract is lost and Synchronize stays blocked until a full apply. No API Object, Procedure, or SDT will be changed. Recover now?",
                ...
                "Delete marcado exige Get, Create e Update nos servicos padrao. Marque os tres ou desmarque Delete." => "A marked Delete requires Get, Create, and Update among the default services. Check all three or clear Delete.",
                "Confirmar" => "Confirm",
