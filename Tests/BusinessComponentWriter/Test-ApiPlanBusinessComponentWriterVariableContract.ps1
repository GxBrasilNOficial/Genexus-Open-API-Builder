...
Assert-Contains $source 'PadL(Trim(Str({member}.Year())), 4, !\"0\")' 'Campos de data na chave primaria devem ser formatados no padrao ISO YYYY-MM-DD no Location header.'
Assert-Contains $source 'EncodeUrl({member}.Trim())' 'Campos de texto na chave primaria devem ser codificados com EncodeUrl no Location header.'
