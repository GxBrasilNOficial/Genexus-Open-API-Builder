...
# B111/F3 P6/P7 — o comando de recuperação e suas mensagens existem nos três idiomas. Um
# comando que só aparece em português é um comando que metade dos usuários não encontra.
Assert-Contains $localization 'Recuperar operação interrompida' 'O comando de recuperação deve existir em português.'
Assert-Contains $localization 'Recuperar operación interrumpida' 'O comando de recuperação deve existir em espanhol.'
Assert-Contains $localization 'Recover interrupted operation' 'O comando de recuperação deve existir em inglês.'
Assert-Contains $localization 'Esta Knowledge Base não tem diário de operação' 'A ausência de diário deve ser explicada em português.'
Assert-Contains $localization 'Esta Knowledge Base no tiene diario de operación' 'A ausência de diário deve ser explicada em espanhol.'
Assert-Contains $localization 'This Knowledge Base has no operation journal' 'A ausência de diário deve ser explicada em inglês.'
Assert-Contains $localization 'Abandoná-la explicitamente libera a KB' 'A confirmação de abandono deve existir em português.'
Assert-Contains $localization 'Abandonarla explícitamente libera la KB' 'A confirmação de abandono deve existir em espanhol.'
Assert-Contains $localization 'Abandoning it explicitly frees the KB' 'A confirmação de abandono deve existir em inglês.'
Assert-Contains $localization 'Retomar usa o mesmo inventário e o mesmo registro' 'A confirmação de retomada deve existir em português.'
Assert-Contains $localization 'Retomar usa el mismo inventario y el mismo registro' 'A confirmação de retomada deve existir em espanhol.'
Assert-Contains $localization 'Resuming uses the same inventory and the same record' 'A confirmação de retomada deve existir em inglês.'
Assert-Contains $localization 'Encerrar este registro libera a KB para as próximas operações' 'O encerramento do registro deve existir em português.'
Assert-Contains $localization 'Cerrar este registro libera la KB para las próximas operaciones' 'O encerramento do registro deve existir em espanhol.'
Assert-Contains $localization 'Closing this record frees the KB for the next operations' 'O encerramento do registro deve existir em inglês.'
Assert-Contains $localization 'foi interrompida antes de gravar qualquer objeto' 'O encerramento sem gravação deve existir em português.'
Assert-Contains $localization 'fue interrumpida antes de grabar cualquier objeto' 'O encerramento sem gravação deve existir em espanhol.'
Assert-Contains $localization 'was interrupted before writing any object' 'O encerramento sem gravação deve existir em inglês.'
Assert-Contains $localization 'Há uma operação interrompida registrada nesta Knowledge Base' 'A oferta proativa deve existir em português.'
Assert-Contains $localization 'Hay una operación interrumpida registrada en esta Knowledge Base' 'A oferta proativa deve existir em espanhol.'
Assert-Contains $localization 'There is an interrupted operation recorded in this Knowledge Base' 'A oferta proativa deve existir em inglês.'
Assert-Equal 'Oferecer a recuperação quando o diário bloquear uma operação' ($uiTerms::PortugueseChrome('Oferecer a recuperacao quando o diario bloquear uma operacao')) 'PT-BR deve acentuar a preferência de oferta da recuperação.'
Assert-Contains $localization 'Ofrecer la recuperación cuando el diario bloquee una operación' 'A preferência de oferta deve existir em espanhol.'
Assert-Contains $localization 'Offer recovery when the journal blocks an operation' 'A preferência de oferta deve existir em inglês.'
