...
Assert-Contains $package 'ReadForIntentionalChangeWithIndex(' 'O Wizard deve validar o baseline sem bloquear mudancas deliberadas no contrato.'
$packageNorm = $package.Replace("`r`n", "`n")
$applyShell = $packageNorm.IndexOf('BusyProgressTitleApply', [StringComparison]::Ordinal)
$applyIndex = $packageNorm.IndexOf('ReadForIntentionalChangeWithIndex(', [StringComparison]::Ordinal)
if ($applyShell -lt 0 -or $applyIndex -lt 0 -or $applyShell -gt $applyIndex) {
    throw 'ASSERT_FAILED: a casca do Apply deve abrir antes do índice da KB.'
}
